...
This R Markdown file details all the data analysis steps used in the manuscript **"Longitudinal Establishment of Human Fecal Bacterial Communities in Porcine and Murine Models"** authored by Aluthge et al.
...
align.seqs(fasta=main_rep_set.fasta, reference=silva.v4_138.fasta)
...
View(mapping_file)
tree_file <- 'main_rep_set.phylip.tre'
...
saveRDS(merged_pig_mouse_comp_ASV_as_seqs_ps, "merged_pig_mouse_comp_ASV_as_seqs_ps.rds") # save phyloseq object with ASVs as sequences
merged_pig_mouse_comp_ASV_as_seqs_ps <- readRDS("merged_pig_mouse_comp_ASV_as_seqs_ps.rds")
write.table(tax_table(merged_pig_mouse_comp_ASV_as_seqs_ps), "ASV_sequence_taxonomy.txt", quote = FALSE, col.names = FALSE, sep = "\t")
...
head(taxa_names(merged_pig_mouse_comp_ps))
saveRDS(merged_pig_mouse_comp_ps, "merged_pig_mouse_comp_ps.rds") # Let's save the phyloseq object with ASV sequences replaced with ASV_IDs
merged_pig_mouse_comp_ps <- readRDS("merged_pig_mouse_comp_ps.rds")
write.table(tax_table(merged_pig_mouse_comp_ps), "ASV_id_taxonomy.txt", quote = FALSE, col.names = FALSE, sep = "\t")
...
write.table(t(otu_table(merged_pig_mouse_comp_complete_ps)), "complete_otu_table.txt", sep = "\t", row.names = TRUE, col.names = NA, quote = FALSE)
```
...
# Let's also make these plots without rarefying since phyloseq authors recommend not rarefying as it 'throws out' data
...
Let's first have a look at how the humanized animal model fecal bacterial communities cluster relative to the human donor inocula communities. As mentioned earlier, we'll compare only the 'late inoculation' piglets to the mice and human donor samples:
...
write.table(otu_table(human_donors_INF2_inocula_relevant_ps, taxa_are_rows = F), "INF2_donors_table.txt", sep = "\t", row.names = T, col.names = NA, quote = F)
...
## Calculating the total number of ASVs in each donor inoculum

# for mice
mouse_inocula <- sample_names(INF2_mouse_inocula_ps) # samples 'R2D269' and 'R2D273'
INF2_mouse_R2D269_inoculum_ps <- subset_samples(INF2_mouse_inocula_ps, SampleID == "R2D269")
INF2_mouse_R2D269_inoculum_ps

prevdf_INF2_R2D269 = apply(X = otu_table(INF2_mouse_R2D269_inoculum_ps),
               MARGIN = ifelse(taxa_are_rows(INF2_mouse_R2D269_inoculum_ps), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})
prevdf_INF2_R2D269 = data.frame(Prevalence=prevdf_INF2_R2D269, TotalAbundance=taxa_sums(INF2_mouse_R2D269_inoculum_ps))
View(prevdf_INF2_R2D269)

all_taxa_INF2_R2D269_inoculum <- rownames(prevdf_INF2_R2D269)[prevdf_INF2_R2D269$Prevalence == 1]
all_taxa_INF2_R2D269_inoculum

INF2_mouse_R2D273_inoculum_ps <- subset_samples(INF2_mouse_inocula_ps, SampleID == "R2D273")
INF2_mouse_R2D273_inoculum_ps

prevdf_INF2_R2D273 = apply(X = otu_table(INF2_mouse_R2D273_inoculum_ps),
               MARGIN = ifelse(taxa_are_rows(INF2_mouse_R2D273_inoculum_ps), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})
prevdf_INF2_R2D273 = data.frame(Prevalence=prevdf_INF2_R2D273, TotalAbundance=taxa_sums(INF2_mouse_R2D273_inoculum_ps))
View(prevdf_INF2_R2D273)

all_taxa_INF2_R2D273_inoculum <- rownames(prevdf_INF2_R2D273)[prevdf_INF2_R2D273$Prevalence == 1]
all_taxa_INF2_R2D273_inoculum # 37 total ASVs detected

# for piglets

prevdf_INF2_piglet_inoculum = apply(X = otu_table(INF2_late_piglet_inoculum_ps),
               MARGIN = ifelse(taxa_are_rows(INF2_late_piglet_inoculum_ps), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})
prevdf_INF2_piglet_inoculum = data.frame(Prevalence=prevdf_INF2_piglet_inoculum, TotalAbundance=taxa_sums(INF2_late_piglet_inoculum_ps))
View(prevdf_INF2_piglet_inoculum)

all_taxa_INF2_piglet_inoculum <- rownames(prevdf_INF2_piglet_inoculum)[prevdf_INF2_piglet_inoculum$Prevalence == 1]
all_taxa_INF2_piglet_inoculum # 31 total ASVs detected
...
# for piglet inoculum
INF2_late_piglet_inoculum_ps <- subset_samples(human_donors_INF2_inocula_relevant_ps, SampleID == '323') # keeps the piglet inoculum
...
sum(otu_table(INF2_late_piglet_inoculum_core_ps))/sum(otu_table(INF2_late_piglet_inoculum_ps)) # the 26 core ASVs represents 98.46% of these donor sequences
```
...
unweighted_pcoa_INF2_core <- plot_ordination(rel_abund_INF2_core_ps, unw_ord_INF2_core, color = "Species", shape = "Species") + ggtitle("Donor_1 PCoA plot - Unweighted UniFrac") + theme(plot.title = element_text(hjust = 0.5, size = 8), legend.position = "none", axis.title = element_text(size = 8)) + labs(colour = "", shape = "host species") #legend.title = element_text(size = 8), legend.text = element_text(size = 8), axis.title = element_text(size = 8)) + labs(colour = "", shape = "host species") 
...
bray_pcoa_INF2_core <- plot_ordination(rel_abund_INF2_core_ps, bray_ord_INF2_core, color = "Species", shape = "Species") + ggtitle("Donor_1 PCoA plot - Bray-Curtis") + theme(plot.title = element_text(hjust = 0.5, size = 8), legend.title = element_text(size = 8), legend.text = element_text(size = 8), axis.title = element_text(size = 8)) + labs(colour = "", shape = "host species") #theme(plot.title = element_text(hjust = 0.5, size = 8), legend.position = "none", axis.title = element_text(size = 8)) + labs(colour = "", shape = "host species") #theme(plot.title = element_text(hjust = 0.5, size = 8), legend.title = element_text(size = 8), legend.text = element_text(size = 8), axis.title = element_text(size = 8)) + labs(colour = "", shape = "host species")
bray_pcoa_INF2_core
...
unwunifrac_INF2_core_by_tp <- plot_ordination(rel_abund_INF2_core_ps, unw_ord_INF2_core, color = "Time_point", shape = "Species") + ggtitle("Donor_1 PCoA plot for core ASVs - unweighted Unifrac") + theme(plot.title = element_text(hjust = 0.5)) 
unwunifrac_INF2_core_by_tp

bray_INF2_by_tp <- plot_ordination(rel_abund_INF2_core_ps, bray_ord_INF2_core, color = "Time_point", shape = "Species") + ggtitle("Donor_1 PCoA plot core ASVs - Bray-Curtis") + theme(plot.title = element_text(hjust = 0.5, size = 10))
bray_INF2_by_tp
...
comparison_test$PLot.zeroes
View(comparison_test)
...
```{r, echo=TRUE}
## using raw counts
#pig_mouse_comp_INF2_comparison_core_ps <- prune_taxa(core_INF2_taxa, pig_mouse_comp_INF2_comparison_ps)
...
## Therefore, we can open "INF2_taxa_table.txt" from above in MS Excel and merge the ASV_ID column with the appropriate taxonomic rank column (for example =A2&" "&F2).
...
asv_tab_w_taxa_INF <- read.table("INF2_ASV_table_heatmap.txt", header = T, sep = "\t", row.names = 1, check.names = F)
...
write.table(sample_data(human_donors_CHLD2_inocula_ps), "CHLD_donors.txt", sep = "\t")
```
...
## Calculating number of total ASVs in each inoculum

# for mice
mouse_inocula <- sample_names(CHLD2_mouse_inocula_ps) # samples 'R2D270' and 'R2D274'
CHLD2_mouse_R2D270_inoculum_ps <- subset_samples(CHLD2_mouse_inocula_ps, SampleID == "R2D270")
CHLD2_mouse_R2D270_inoculum_ps

prevdf_CHLD2_R2D270 = apply(X = otu_table(CHLD2_mouse_R2D270_inoculum_ps),
               MARGIN = ifelse(taxa_are_rows(CHLD2_mouse_R2D270_inoculum_ps), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})
prevdf_CHLD2_R2D270 = data.frame(Prevalence=prevdf_CHLD2_R2D270, TotalAbundance=taxa_sums(CHLD2_mouse_R2D270_inoculum_ps))
View(prevdf_CHLD2_R2D270)

all_taxa_CHLD2_R2D270_inoculum <- rownames(prevdf_CHLD2_R2D270)[prevdf_CHLD2_R2D270$Prevalence == 1]
all_taxa_CHLD2_R2D270_inoculum # 107 total ASVs

CHLD2_mouse_R2D274_inoculum_ps <- subset_samples(CHLD2_mouse_inocula_ps, SampleID == "R2D274")
CHLD2_mouse_R2D274_inoculum_ps 

prevdf_CHLD2_R2D274 = apply(X = otu_table(CHLD2_mouse_R2D274_inoculum_ps),
               MARGIN = ifelse(taxa_are_rows(CHLD2_mouse_R2D274_inoculum_ps), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})
prevdf_CHLD2_R2D274 = data.frame(Prevalence=prevdf_CHLD2_R2D274, TotalAbundance=taxa_sums(CHLD2_mouse_R2D274_inoculum_ps))
View(prevdf_CHLD2_R2D274)

all_taxa_CHLD2_R2D274_inoculum <- rownames(prevdf_CHLD2_R2D274)[prevdf_CHLD2_R2D274$Prevalence == 1]
all_taxa_CHLD2_R2D274_inoculum # 117 total ASVs

# for piglets
piglet_inocula <- sample_names(CHLD2_piglet_inocula_ps) # samples '360' and '371'
CHLD2_piglet_360_inoculum_ps <- subset_samples(CHLD2_piglet_inocula_ps, SampleID == "360")
CHLD2_piglet_360_inoculum_ps

prevdf_CHLD2_360 = apply(X = otu_table(CHLD2_piglet_360_inoculum_ps),
               MARGIN = ifelse(taxa_are_rows(CHLD2_piglet_360_inoculum_ps), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})
prevdf_CHLD2_360 = data.frame(Prevalence=prevdf_CHLD2_360, TotalAbundance=taxa_sums(CHLD2_piglet_360_inoculum_ps))
View(prevdf_CHLD2_360)

all_taxa_CHLD2_360_inoculum <- rownames(prevdf_CHLD2_360)[prevdf_CHLD2_360$Prevalence == 1]
all_taxa_CHLD2_360_inoculum # 153 ASVs detected


CHLD2_piglet_371_inoculum_ps <- subset_samples(CHLD2_piglet_inocula_ps, SampleID == "371")
CHLD2_piglet_371_inoculum_ps 

prevdf_CHLD2_371 = apply(X = otu_table(CHLD2_piglet_371_inoculum_ps),
               MARGIN = ifelse(taxa_are_rows(CHLD2_piglet_371_inoculum_ps), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})
prevdf_CHLD2_371 = data.frame(Prevalence=prevdf_CHLD2_371, TotalAbundance=taxa_sums(CHLD2_piglet_371_inoculum_ps))
View(prevdf_CHLD2_371)

all_taxa_CHLD2_371_inoculum <- rownames(prevdf_CHLD2_371)[prevdf_CHLD2_371$Prevalence == 1]
all_taxa_CHLD2_371_inoculum # 87 total ASVs


# Calculating how much of each donor inocula group was represented by these core ASVs
# for mouse inoculae
CHLD2_mouse_inocula_IDs <- c('R2D270','R2D274')
CHLD2_piglet_inocula_IDs <- c('360','371')

CHLD2_mouse_inocula_ps <- subset_samples(human_donors_CHLD2_inocula_ps, !SampleID %in% CHLD2_piglet_inocula_IDs) # removes piglet inoculum
CHLD2_mouse_inocula_ps
CHLD2_mouse_inocula_core_ps <- prune_taxa(core_CHLD2_taxa,CHLD2_mouse_inocula_ps)
CHLD2_mouse_inocula_core_ps
sum(otu_table(CHLD2_mouse_inocula_core_ps))/sum(otu_table(CHLD2_mouse_inocula_ps)) # the 76 core ASVs represents 97.13% of these donor sequences

# for piglet inoculae
CHLD2_piglet_inocula_ps <- subset_samples(human_donors_CHLD2_inocula_ps, !SampleID %in% CHLD2_mouse_inocula_IDs) # removes piglet inoculum
CHLD2_piglet_inocula_ps
CHLD2_piglet_inocula_core_ps <- prune_taxa(core_CHLD2_taxa,CHLD2_piglet_inocula_ps)
CHLD2_piglet_inocula_core_ps
sum(otu_table(CHLD2_piglet_inocula_core_ps))/sum(otu_table(CHLD2_piglet_inocula_ps)) # the 76 core ASVs represents 95.84% of these donor sequences
```
...
## Let's also color the unweighted unifrac and bray-curtis plots by sampling time point
unwunifrac_CHLD2_core_by_tp <- plot_ordination(rel_abund_CHLD2_core_ps, unw_ord_CHLD2_core, color = "Time_point", shape = "Species") + ggtitle("Donor_2 PCoA plot for core ASVs - unweighted unifrac") + theme(plot.title = element_text(hjust = 0.5)) 
unwunifrac_CHLD2_core_by_tp

bray_CHLD2_by_tp <- plot_ordination(rel_abund_CHLD2_core_ps, bray_ord_CHLD2_core, color = "Time_point", shape = "Species") + ggtitle("Donor_2 PCoA plot core ASVs - Bray-Curtis") + theme(plot.title = element_text(hjust = 0.5))
bray_CHLD2_by_tp
...
```{r, echo=TRUE}
# using raw counts
#pig_mouse_comp_CHLD2_comparison_core_ps <- prune_taxa(core_CHLD2_taxa,pig_mouse_comp_CHLD2_comparison_ps)
...
asv_tab_w_taxa_CHLD <- read.table("CHLD2_ASV_table_heatmap.txt", header = T, sep = "\t", row.names = 1, check.names = F)
...
# for mice
mouse_inocula <- sample_names(ADLT2_mouse_inocula_ps) # samples 'R2D271' and 'R2D275'
ADLT2_mouse_R2D271_inoculum_ps <- subset_samples(ADLT2_mouse_inocula_ps, SampleID == "R2D271")
ADLT2_mouse_R2D271_inoculum_ps

prevdf_ADLT2_R2D271 = apply(X = otu_table(ADLT2_mouse_R2D271_inoculum_ps),
               MARGIN = ifelse(taxa_are_rows(ADLT2_mouse_R2D271_inoculum_ps), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})
prevdf_ADLT2_R2D271 = data.frame(Prevalence=prevdf_ADLT2_R2D271, TotalAbundance=taxa_sums(ADLT2_mouse_R2D271_inoculum_ps))
View(prevdf_ADLT2_R2D271)

all_taxa_ADLT2_R2D271_inoculum <- rownames(prevdf_ADLT2_R2D271)[prevdf_ADLT2_R2D271$Prevalence == 1]
all_taxa_ADLT2_R2D271_inoculum # 175 total ASVs

ADLT2_mouse_R2D275_inoculum_ps <- subset_samples(ADLT2_mouse_inocula_ps, SampleID == "R2D275")
ADLT2_mouse_R2D275_inoculum_ps 

prevdf_ADLT2_R2D275 = apply(X = otu_table(ADLT2_mouse_R2D275_inoculum_ps),
               MARGIN = ifelse(taxa_are_rows(ADLT2_mouse_R2D275_inoculum_ps), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})
prevdf_ADLT2_R2D275 = data.frame(Prevalence=prevdf_ADLT2_R2D275, TotalAbundance=taxa_sums(ADLT2_mouse_R2D275_inoculum_ps))
View(prevdf_ADLT2_R2D275)

all_taxa_ADLT2_R2D275_inoculum <- rownames(prevdf_ADLT2_R2D275)[prevdf_ADLT2_R2D275$Prevalence == 1]
all_taxa_ADLT2_R2D275_inoculum # 202 total ASVs

# for piglets
piglet_inocula <- sample_names(ADLT2_piglet_inocula_ps) # samples '348' and '372'
ADLT2_piglet_348_inoculum_ps <- subset_samples(ADLT2_piglet_inocula_ps, SampleID == "348")
ADLT2_piglet_348_inoculum_ps

prevdf_ADLT2_348 = apply(X = otu_table(ADLT2_piglet_348_inoculum_ps),
               MARGIN = ifelse(taxa_are_rows(ADLT2_piglet_348_inoculum_ps), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})
prevdf_ADLT2_348 = data.frame(Prevalence=prevdf_ADLT2_348, TotalAbundance=taxa_sums(ADLT2_piglet_348_inoculum_ps))
View(prevdf_ADLT2_348)

all_taxa_ADLT2_348_inoculum <- rownames(prevdf_ADLT2_348)[prevdf_ADLT2_348$Prevalence == 1]
all_taxa_ADLT2_348_inoculum # 177 ASVs detected


ADLT2_piglet_372_inoculum_ps <- subset_samples(ADLT2_piglet_inocula_ps, SampleID == "372")
ADLT2_piglet_372_inoculum_ps 

prevdf_ADLT2_372 = apply(X = otu_table(ADLT2_piglet_372_inoculum_ps),
               MARGIN = ifelse(taxa_are_rows(ADLT2_piglet_372_inoculum_ps), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})
prevdf_ADLT2_372 = data.frame(Prevalence=prevdf_ADLT2_372, TotalAbundance=taxa_sums(ADLT2_piglet_372_inoculum_ps))
View(prevdf_ADLT2_372)

all_taxa_ADLT2_372_inoculum <- rownames(prevdf_ADLT2_372)[prevdf_ADLT2_372$Prevalence == 1]
all_taxa_ADLT2_372_inoculum # 205 total ASVs



# Calculating how much of each donor inocula group was represented by these core ASVs
# for mouse inoculae
ADLT2_mouse_inocula_IDs <- c('R2D271','R2D275')
ADLT2_piglet_inocula_IDs <- c('348', '372')

ADLT2_mouse_inocula_ps <- subset_samples(human_donors_ADLT2_inocula_ps, !SampleID %in% ADLT2_piglet_inocula_IDs) # removes piglet inoculum
sample_names(ADLT2_mouse_inocula_ps)
ADLT2_mouse_inocula_core_ps <- prune_taxa(core_ADLT2_taxa,ADLT2_mouse_inocula_ps)
ADLT2_mouse_inocula_core_ps
sum(otu_table(ADLT2_mouse_inocula_core_ps))/sum(otu_table(ADLT2_mouse_inocula_ps)) # the 140 core ASVs represents 97.73% of these donor sequences

# for piglet inoculae
ADLT2_piglet_inocula_ps <- subset_samples(human_donors_ADLT2_inocula_ps, !SampleID %in% ADLT2_mouse_inocula_IDs) # removes piglet inoculum
ADLT2_piglet_inocula_ps
ADLT2_piglet_inocula_core_ps <- prune_taxa(core_ADLT2_taxa,ADLT2_piglet_inocula_ps)
ADLT2_piglet_inocula_core_ps
sum(otu_table(ADLT2_piglet_inocula_core_ps))/sum(otu_table(ADLT2_piglet_inocula_ps)) # the 140 core ASVs represents 97.41% of these donor sequences
```
...
sample_names(rel_abund_ADLT2_core_ps)
...
## Let's also color the unweighted unifrac and bray-curtis plots by sampling time point
unwunifrac_ADLT2_core_by_tp <- plot_ordination(rel_abund_ADLT2_core_ps, unw_ord_ADLT2_core, color = "Time_point", shape = "Species") + ggtitle("Donor_3 PCoA plot for core ASVs - unweighted unifrac") + theme(plot.title = element_text(hjust = 0.5)) 
unwunifrac_ADLT2_core_by_tp

bray_ADLT2_by_tp <- plot_ordination(rel_abund_ADLT2_core_ps, bray_ord_ADLT2_core, color = "Time_point", shape = "Species") + ggtitle("Donor_3 PCoA plot core ASVs - Bray-Curtis") + theme(plot.title = element_text(hjust = 0.5))
bray_ADLT2_by_tp
...
```{r, echo=TRUE}
# using raw counts
#pig_mouse_comp_ADLT2_comparison_core_ps <- prune_taxa(core_ADLT2_taxa, pig_mouse_comp_ADLT2_comparison_ps)
...
asv_tab_w_taxa_ADLT <- read.table("ADLT2_ASV_table_heatmap.txt", header = T, sep = "\t", row.names = 1, check.names = F)
...
**Donor_4 (Elderly/Senior) Analysis**
...
# for mice
mouse_inocula <- sample_names(ELD2_mouse_inocula_ps) # samples 'R2D272' and 'R2D276'
ELD2_mouse_R2D272_inoculum_ps <- subset_samples(ELD2_mouse_inocula_ps, SampleID == "R2D272")
ELD2_mouse_R2D272_inoculum_ps

prevdf_ELD2_R2D272 = apply(X = otu_table(ELD2_mouse_R2D272_inoculum_ps),
               MARGIN = ifelse(taxa_are_rows(ELD2_mouse_R2D272_inoculum_ps), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})
prevdf_ELD2_R2D272 = data.frame(Prevalence=prevdf_ELD2_R2D272, TotalAbundance=taxa_sums(ELD2_mouse_R2D272_inoculum_ps))
View(prevdf_ELD2_R2D272)

all_taxa_ELD2_R2D272_inoculum <- rownames(prevdf_ELD2_R2D272)[prevdf_ELD2_R2D272$Prevalence == 1]
all_taxa_ELD2_R2D272_inoculum # 179 total ASVs

ELD2_mouse_R2D276_inoculum_ps <- subset_samples(ELD2_mouse_inocula_ps, SampleID == "R2D276")
ELD2_mouse_R2D276_inoculum_ps 

prevdf_ELD2_R2D276 = apply(X = otu_table(ELD2_mouse_R2D276_inoculum_ps),
               MARGIN = ifelse(taxa_are_rows(ELD2_mouse_R2D276_inoculum_ps), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})
prevdf_ELD2_R2D276 = data.frame(Prevalence=prevdf_ELD2_R2D276, TotalAbundance=taxa_sums(ELD2_mouse_R2D276_inoculum_ps))
View(prevdf_ELD2_R2D276)

all_taxa_ELD2_R2D276_inoculum <- rownames(prevdf_ELD2_R2D276)[prevdf_ELD2_R2D276$Prevalence == 1]
all_taxa_ELD2_R2D276_inoculum # 169 total ASVs

# for piglets
piglet_inocula <- sample_names(ELD2_piglet_inocula_ps) # samples '324' and '359'
ELD2_piglet_324_inoculum_ps <- subset_samples(ELD2_piglet_inocula_ps, SampleID == "324")
ELD2_piglet_324_inoculum_ps

prevdf_ELD2_324 = apply(X = otu_table(ELD2_piglet_324_inoculum_ps),
               MARGIN = ifelse(taxa_are_rows(ELD2_piglet_324_inoculum_ps), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})
prevdf_ELD2_324 = data.frame(Prevalence=prevdf_ELD2_324, TotalAbundance=taxa_sums(ELD2_piglet_324_inoculum_ps))
View(prevdf_ELD2_324)

all_taxa_ELD2_324_inoculum <- rownames(prevdf_ELD2_324)[prevdf_ELD2_324$Prevalence == 1]
all_taxa_ELD2_324_inoculum # 278 ASVs detected


ELD2_piglet_359_inoculum_ps <- subset_samples(ELD2_piglet_inocula_ps, SampleID == "359")
ELD2_piglet_359_inoculum_ps 

prevdf_ELD2_359 = apply(X = otu_table(ELD2_piglet_359_inoculum_ps),
               MARGIN = ifelse(taxa_are_rows(ELD2_piglet_359_inoculum_ps), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})
prevdf_ELD2_359 = data.frame(Prevalence=prevdf_ELD2_359, TotalAbundance=taxa_sums(ELD2_piglet_359_inoculum_ps))
View(prevdf_ELD2_359)

all_taxa_ELD2_359_inoculum <- rownames(prevdf_ELD2_359)[prevdf_ELD2_359$Prevalence == 1]
all_taxa_ELD2_359_inoculum # 205 total ASVs

# Calculating how much of each donor inocula group was represented by these core ASVs
# for mouse inoculae
ELD2_mouse_inocula_IDs <- c('R2D272','R2D276')
ELD2_piglet_inocula_IDs <- c('324', '359')

ELD2_mouse_inocula_ps <- subset_samples(human_donors_ELD2_inocula_ps, !SampleID %in% ELD2_piglet_inocula_IDs) # removes piglet inoculum
sample_names(ELD2_mouse_inocula_ps)
ELD2_mouse_inocula_core_ps <- prune_taxa(core_ELD2_taxa,ELD2_mouse_inocula_ps)
ELD2_mouse_inocula_core_ps
sum(otu_table(ELD2_mouse_inocula_core_ps))/sum(otu_table(ELD2_mouse_inocula_ps)) # the 134 core ASVs represents 98.06% of these donor sequences

# for piglet inoculae
ELD2_piglet_inocula_ps <- subset_samples(human_donors_ELD2_inocula_ps, !SampleID %in% ELD2_mouse_inocula_IDs) # removes piglet inoculum
ELD2_piglet_inocula_ps
ELD2_piglet_inocula_core_ps <- prune_taxa(core_ELD2_taxa,ELD2_piglet_inocula_ps)
ELD2_piglet_inocula_core_ps
sum(otu_table(ELD2_piglet_inocula_core_ps))/sum(otu_table(ELD2_piglet_inocula_ps)) # the 140 core ASVs represents 94.96% of these donor sequences
```
...
## Let's also color the unweighted unifrac and bray-curtis plots by sampling time point
unwunifrac_ELD2_core_by_tp <- plot_ordination(rel_abund_ELD2_core_ps, unw_ord_ELD2_core, color = "Time_point", shape = "Species") + ggtitle("Donor_4 PCoA plot core ASVs - Unweighted Unifrac") + theme(plot.title = element_text(hjust = 0.5)) 
unwunifrac_ELD2_core_by_tp

bray_ELD2_by_tp <- plot_ordination(rel_abund_ELD2_core_ps, bray_ord_ELD2_core, color = "Time_point", shape = "Species") + ggtitle("Donor_4 PCoA plot core ASVs - Bray-Curtis") + theme(plot.title = element_text(hjust = 0.5))
bray_ELD2_by_tp
...
```{r, echo=TRUE}
# using raw counts
#pig_mouse_comp_ELD2_comparison_core_ps <- prune_taxa(core_ELD2_taxa, pig_mouse_comp_ELD2_comparison_ps)
...
asv_tab_w_taxa_ELD <- read.table("ELD2_ASV_table_heatmap.txt", header = T, sep = "\t", row.names = 1, check.names = F)
...
Let's see how many of these core ASVs have established in the HMA animal models. To be a colonizer, an ASV needs to be found in at least a single fecal sample. In order to be considered 'persistently colonizing', an ASV needs to be detected in at least **4** samplings and in at least **2/3 of animals** :
...
View(colonized_prevdf_CHLD2_core_piglets) # 65 of the 76 core CHLD2 donor ASVs found in at least one CHLD2 mouse fecal sample. These are the 'COLONIZERS'.
...
View(colonized_prevdf_ADLT2_core_piglets) # 106 of the 140 core ADLT2 donor ASVs found in at least one ADLT2 mouse fecal sample. These are the 'COLONIZERS'
...
## **Looking at the taxonomic classifications to which the core persistent colonizers belong to** ##
...
# at PHYLUM level
INF2_perst_core_taxa_mouse_table_PHYLUM <- table(tax_table(perst_colonizing_core_INF2_mice_ps)[, "Phylum"], exclude = NULL)
...
# at FAMILY level
INF2_perst_core_taxa_mouse_table_FAMILY <- table(tax_table(perst_colonizing_core_INF2_mice_ps)[, "Family"], exclude = NULL)
...
# at GENUS level
INF2_perst_core_taxa_mouse_table_GENUS <- table(tax_table(perst_colonizing_core_INF2_mice_ps)[, "Genus"], exclude = NULL)
...
perst_colonizing_core_INF2_mice_ps
# Compute prevalence of each feature, store as data.frame
prevdf_mouse_INF2 = apply(X = otu_table(perst_colonizing_core_INF2_mice_ps),
               MARGIN = ifelse(taxa_are_rows(perst_colonizing_core_INF2_mice_ps), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})
# Add taxonomy and total read counts to this data.frame
prevdf_mouse_INF2 = data.frame(Prevalence = prevdf_mouse_INF2,
                    TotalAbundance = taxa_sums(perst_colonizing_core_INF2_mice_ps),
                    tax_table(perst_colonizing_core_INF2_mice_ps))
...
# at PHYLUM level
INF2_perst_core_taxa_piglets_table_PHYLUM <- table(tax_table(perst_colonizing_core_INF2_piglets_ps)[, "Phylum"], exclude = NULL)
...
# at FAMILY level
INF2_perst_core_taxa_piglets_table_FAMILY <- table(tax_table(perst_colonizing_core_INF2_piglets_ps)[, "Family"], exclude = NULL)
...
# at GENUS level
INF2_perst_core_taxa_piglets_table_GENUS <- table(tax_table(perst_colonizing_core_INF2_piglets_ps)[, "Genus"], exclude = NULL)
...
perst_colonizing_core_INF2_piglets_ps
# Compute prevalence of each feature, store as data.frame
prevdf_piglets_INF2 = apply(X = otu_table(perst_colonizing_core_INF2_piglets_ps),
               MARGIN = ifelse(taxa_are_rows(perst_colonizing_core_INF2_piglets_ps), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})
# Add taxonomy and total read counts to this data.frame
prevdf_piglets_INF2 = data.frame(Prevalence = prevdf_piglets_INF2,
                    TotalAbundance = taxa_sums(perst_colonizing_core_INF2_piglets_ps),
                    tax_table(perst_colonizing_core_INF2_piglets_ps))
...
## MERGED taxa level tables for INF2 donor
...
# at PHYLUM level
CHLD2_perst_core_taxa_mouse_table_PHYLUM <- table(tax_table(perst_colonizing_core_CHLD2_mice_ps)[, "Phylum"], exclude = NULL)
...
# at FAMILY level
CHLD2_perst_core_taxa_mouse_table_FAMILY <- table(tax_table(perst_colonizing_core_CHLD2_mice_ps)[, "Family"], exclude = NULL)
...
# at GENUS level
CHLD2_perst_core_taxa_mouse_table_GENUS <- table(tax_table(perst_colonizing_core_CHLD2_mice_ps)[, "Genus"], exclude = NULL)
...
perst_colonizing_core_CHLD2_mice_ps
# Compute prevalence of each feature, store as data.frame
prevdf_mouse_CHLD2 = apply(X = otu_table(perst_colonizing_core_CHLD2_mice_ps),
               MARGIN = ifelse(taxa_are_rows(perst_colonizing_core_CHLD2_mice_ps), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})
# Add taxonomy and total read counts to this data.frame
prevdf_mouse_CHLD2 = data.frame(Prevalence = prevdf_mouse_CHLD2,
                    TotalAbundance = taxa_sums(perst_colonizing_core_CHLD2_mice_ps),
                    tax_table(perst_colonizing_core_CHLD2_mice_ps))
...
# at PHYLUM level
CHLD2_perst_core_taxa_piglets_table_PHYLUM <- table(tax_table(perst_colonizing_core_CHLD2_piglets_ps)[, "Phylum"], exclude = NULL)
...
# at FAMILY level
CHLD2_perst_core_taxa_piglets_table_FAMILY <- table(tax_table(perst_colonizing_core_CHLD2_piglets_ps)[, "Family"], exclude = NULL)
...
# at GENUS level
CHLD2_perst_core_taxa_piglets_table_GENUS <- table(tax_table(perst_colonizing_core_CHLD2_piglets_ps)[, "Genus"], exclude = NULL)
...
perst_colonizing_core_CHLD2_piglets_ps
# Compute prevalence of each feature, store as data.frame
prevdf_piglets_CHLD2 = apply(X = otu_table(perst_colonizing_core_CHLD2_piglets_ps),
               MARGIN = ifelse(taxa_are_rows(perst_colonizing_core_CHLD2_piglets_ps), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})
# Add taxonomy and total read counts to this data.frame
prevdf_piglets_CHLD2 = data.frame(Prevalence = prevdf_piglets_CHLD2,
                    TotalAbundance = taxa_sums(perst_colonizing_core_CHLD2_piglets_ps),
                    tax_table(perst_colonizing_core_CHLD2_piglets_ps))
...
# at PHYLUM level
ADLT2_perst_core_taxa_mouse_table_PHYLUM <- table(tax_table(perst_colonizing_core_ADLT2_mice_ps)[, "Phylum"], exclude = NULL)
...
# at FAMILY level
ADLT2_perst_core_taxa_mouse_table_FAMILY <- table(tax_table(perst_colonizing_core_ADLT2_mice_ps)[, "Family"], exclude = NULL)
...
# at GENUS level
ADLT2_perst_core_taxa_mouse_table_GENUS <- table(tax_table(perst_colonizing_core_ADLT2_mice_ps)[, "Genus"], exclude = NULL)
...
perst_colonizing_core_ADLT2_mice_ps
# Compute prevalence of each feature, store as data.frame
prevdf_mouse_ADLT2 = apply(X = otu_table(perst_colonizing_core_ADLT2_mice_ps),
               MARGIN = ifelse(taxa_are_rows(perst_colonizing_core_ADLT2_mice_ps), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})
# Add taxonomy and total read counts to this data.frame
prevdf_mouse_ADLT2 = data.frame(Prevalence = prevdf_mouse_ADLT2,
                    TotalAbundance = taxa_sums(perst_colonizing_core_ADLT2_mice_ps),
                    tax_table(perst_colonizing_core_ADLT2_mice_ps))
...
# at PHYLUM level
ADLT2_perst_core_taxa_piglets_table_PHYLUM <- table(tax_table(perst_colonizing_core_ADLT2_piglets_ps)[, "Phylum"], exclude = NULL)
...
# at FAMILY level
ADLT2_perst_core_taxa_piglets_table_FAMILY <- table(tax_table(perst_colonizing_core_ADLT2_piglets_ps)[, "Family"], exclude = NULL)
...
# at GENUS level
ADLT2_perst_core_taxa_piglets_table_GENUS <- table(tax_table(perst_colonizing_core_ADLT2_piglets_ps)[, "Genus"], exclude = NULL)
...
perst_colonizing_core_ADLT2_piglets_ps
# Compute prevalence of each feature, store as data.frame
prevdf_piglets_ADLT2 = apply(X = otu_table(perst_colonizing_core_ADLT2_piglets_ps),
               MARGIN = ifelse(taxa_are_rows(perst_colonizing_core_ADLT2_piglets_ps), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})
# Add taxonomy and total read counts to this data.frame
prevdf_piglets_ADLT2 = data.frame(Prevalence = prevdf_piglets_ADLT2,
                    TotalAbundance = taxa_sums(perst_colonizing_core_ADLT2_piglets_ps),
                    tax_table(perst_colonizing_core_ADLT2_piglets_ps))
...
# at PHYLUM level
ELD2_perst_core_taxa_mouse_table_PHYLUM <- table(tax_table(perst_colonizing_core_ELD2_mice_ps)[, "Phylum"], exclude = NULL)
...
# at FAMILY level
ELD2_perst_core_taxa_mouse_table_FAMILY <- table(tax_table(perst_colonizing_core_ELD2_mice_ps)[, "Family"], exclude = NULL)
...
# at GENUS level
ELD2_perst_core_taxa_mouse_table_GENUS <- table(tax_table(perst_colonizing_core_ELD2_mice_ps)[, "Genus"], exclude = NULL)
...
perst_colonizing_core_ELD2_mice_ps
# Compute prevalence of each feature, store as data.frame
prevdf_mouse_ELD2 = apply(X = otu_table(perst_colonizing_core_ELD2_mice_ps),
               MARGIN = ifelse(taxa_are_rows(perst_colonizing_core_ELD2_mice_ps), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})
# Add taxonomy and total read counts to this data.frame
prevdf_mouse_ELD2 = data.frame(Prevalence = prevdf_mouse_ELD2,
                    TotalAbundance = taxa_sums(perst_colonizing_core_ELD2_mice_ps),
                    tax_table(perst_colonizing_core_ELD2_mice_ps))
...
# at PHYLUM level
ELD2_perst_core_taxa_piglets_table_PHYLUM <- table(tax_table(perst_colonizing_core_ELD2_piglets_ps)[, "Phylum"], exclude = NULL)
...
# at FAMILY level
ELD2_perst_core_taxa_piglets_table_FAMILY <- table(tax_table(perst_colonizing_core_ELD2_piglets_ps)[, "Family"], exclude = NULL)
...
# at GENUS level
ELD2_perst_core_taxa_piglets_table_GENUS <- table(tax_table(perst_colonizing_core_ELD2_piglets_ps)[, "Genus"], exclude = NULL)
...
perst_colonizing_core_ELD2_piglets_ps
# Compute prevalence of each feature, store as data.frame
prevdf_piglets_ELD2 = apply(X = otu_table(perst_colonizing_core_ELD2_piglets_ps),
               MARGIN = ifelse(taxa_are_rows(perst_colonizing_core_ELD2_piglets_ps), yes = 1, no = 2),
               FUN = function(x){sum(x > 0)})
# Add taxonomy and total read counts to this data.frame
prevdf_piglets_ELD2 = data.frame(Prevalence = prevdf_piglets_ELD2,
                    TotalAbundance = taxa_sums(perst_colonizing_core_ELD2_piglets_ps),
                    tax_table(perst_colonizing_core_ELD2_piglets_ps))
...
INF2_HMA_mice_ps
INF2_colonizers_ps <- prune_taxa(perst_colonizing_core_INF2_mice,INF2_HMA_mice_ps)
INF2_colonizers_ps

# at PHYLUM level
INF2_perst_core_taxa_PHYLUM <- table(tax_table(INF2_colonizers_ps)[, "Phylum"], exclude = NULL)
...
colnames(INF2_perst_core_taxa_PHYLUM_table_mice)[colnames(INF2_perst_core_taxa_PHYLUM_table_mice) == "Freq"] <- "number of ASVs in mice"
...
# at FAMILY level
INF2_perst_core_taxa_FAMILY <- table(tax_table(INF2_colonizers_ps)[, "Family"], exclude = NULL)
...
colnames(INF2_perst_core_taxa_FAMILY_table_mice)[colnames(INF2_perst_core_taxa_FAMILY_table_mice) == "Freq"] <- "number of ASVs in mice"
...
# at GENUS level
INF2_perst_core_taxa_GENUS <- table(tax_table(INF2_colonizers_ps)[, "Genus"], exclude = NULL)
...
colnames(INF2_perst_core_taxa_GENUS_table_mice)[colnames(INF2_perst_core_taxa_GENUS_table_mice) == "Freq"] <- "number of ASVs in mice"
...
INF2_HMA_piglets_ps
INF2_coreonizers_ps <- prune_taxa(perst_colonizing_core_INF2_piglets,INF2_HMA_piglets_ps)
INF2_coreonizers_ps

# at PHYLUM level
INF2_perst_core_taxa_PHYLUM <- table(tax_table(INF2_coreonizers_ps)[, "Phylum"], exclude = NULL)
...
colnames(INF2_perst_core_taxa_PHYLUM_table_piglets)[colnames(INF2_perst_core_taxa_PHYLUM_table_piglets) == "Freq"] <- "number of ASVs in piglets"
...
# at FAMILY level
INF2_perst_core_taxa_FAMILY <- table(tax_table(INF2_coreonizers_ps)[, "Family"], exclude = NULL)
...
colnames(INF2_perst_core_taxa_FAMILY_table_piglets)[colnames(INF2_perst_core_taxa_FAMILY_table_piglets) == "Freq"] <- "number of ASVs in piglets"
...
# at GENUS level
INF2_perst_core_taxa_GENUS <- table(tax_table(INF2_coreonizers_ps)[, "Genus"], exclude = NULL)
...
colnames(INF2_perst_core_taxa_GENUS_table_piglets)[colnames(INF2_perst_core_taxa_GENUS_table_piglets) == "Freq"] <- "number of ASVs in piglets"
...
# Merging info for all the species
human_donors_INF2_PHYLUM_table
merged_INF2_perst_coloniz_PHYLUM <- Reduce(function(x, y) merge(x, y, all=TRUE),list(human_donors_INF2_PHYLUM_table,INF2_perst_core_taxa_PHYLUM_table_mice,INF2_perst_core_taxa_PHYLUM_table_piglets))
View(merged_INF2_perst_coloniz_PHYLUM)
write.table(merged_INF2_perst_coloniz_PHYLUM, "INF2_perst_colonizer_taxa_breakdown_PHYLUM.txt", sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)

human_donors_INF2_FAMILY_table
merged_INF2_perst_coloniz_FAMILY <- Reduce(function(x, y) merge(x, y, all=TRUE),list(human_donors_INF2_FAMILY_table,INF2_perst_core_taxa_FAMILY_table_mice,INF2_perst_core_taxa_FAMILY_table_piglets))
View(merged_INF2_perst_coloniz_FAMILY)
write.table(merged_INF2_perst_coloniz_FAMILY, "INF2_perst_colonizer_taxa_breakdown_FAMILY.txt", sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)

human_donors_INF2_GENUS_table
merged_INF2_perst_coloniz_GENUS <- Reduce(function(x, y) merge(x, y, all=TRUE),list(human_donors_INF2_GENUS_table,INF2_perst_core_taxa_GENUS_table_mice,INF2_perst_core_taxa_GENUS_table_piglets))
View(merged_INF2_perst_coloniz_GENUS)
write.table(merged_INF2_perst_coloniz_GENUS, "INF2_perst_colonizer_taxa_breakdown_GENUS.txt", sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)
```
...
CHLD2_HMA_mice_ps
CHLD2_colonizers_mice_ps <- prune_taxa(perst_colonizing_core_CHLD2_mice,CHLD2_HMA_mice_ps)
CHLD2_colonizers_mice_ps

# at PHYLUM level
CHLD2_perst_core_taxa_PHYLUM <- table(tax_table(CHLD2_colonizers_mice_ps)[, "Phylum"], exclude = NULL)
...
colnames(CHLD2_perst_core_taxa_PHYLUM_table_mice)[colnames(CHLD2_perst_core_taxa_PHYLUM_table_mice) == "Freq"] <- "number of ASVs in mice"
...
# at FAMILY level
CHLD2_perst_core_taxa_FAMILY <- table(tax_table(CHLD2_colonizers_mice_ps)[, "Family"], exclude = NULL)
...
colnames(CHLD2_perst_core_taxa_FAMILY_table_mice)[colnames(CHLD2_perst_core_taxa_FAMILY_table_mice) == "Freq"] <- "number of ASVs in mice"
...
# at GENUS level
CHLD2_perst_core_taxa_GENUS <- table(tax_table(CHLD2_colonizers_mice_ps)[, "Genus"], exclude = NULL)
...
colnames(CHLD2_perst_core_taxa_GENUS_table_mice)[colnames(CHLD2_perst_core_taxa_GENUS_table_mice) == "Freq"] <- "number of ASVs in mice"
...
CHLD2_HMA_piglets_ps
CHLD2_colonizers_piglets_ps <- prune_taxa(perst_colonizing_core_CHLD2_piglets,CHLD2_HMA_piglets_ps)
CHLD2_colonizers_piglets_ps

# at PHYLUM level
CHLD2_perst_core_taxa_PHYLUM <- table(tax_table(CHLD2_colonizers_piglets_ps)[, "Phylum"], exclude = NULL)
...
colnames(CHLD2_perst_core_taxa_PHYLUM_table_piglets)[colnames(CHLD2_perst_core_taxa_PHYLUM_table_piglets) == "Freq"] <- "number of ASVs piglets"
...
# at FAMILY level
CHLD2_perst_core_taxa_FAMILY <- table(tax_table(CHLD2_colonizers_piglets_ps)[, "Family"], exclude = NULL)
...
colnames(CHLD2_perst_core_taxa_FAMILY_table_piglets)[colnames(CHLD2_perst_core_taxa_FAMILY_table_piglets) == "Freq"] <- "number of ASVs piglets"
...
# at GENUS level
CHLD2_perst_core_taxa_GENUS <- table(tax_table(CHLD2_colonizers_piglets_ps)[, "Genus"], exclude = NULL)
...
colnames(CHLD2_perst_core_taxa_GENUS_table_piglets)[colnames(CHLD2_perst_core_taxa_GENUS_table_piglets) == "Freq"] <- "number of ASVs piglets"
...
# Merging info for all three species
human_donors_CHLD2_PHYLUM_table
merged_CHLD2_perst_coloniz_PHYLUM <- Reduce(function(x, y) merge(x, y, all=TRUE),list(human_donors_CHLD2_PHYLUM_table,CHLD2_perst_core_taxa_PHYLUM_table_mice,CHLD2_perst_core_taxa_PHYLUM_table_piglets))
View(merged_CHLD2_perst_coloniz_PHYLUM)
write.table(merged_CHLD2_perst_coloniz_PHYLUM, "CHLD2_perst_colonizer_taxa_breakdown_PHYLUM.txt", sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)

human_donors_CHLD2_FAMILY_table
merged_CHLD2_perst_coloniz_FAMILY <- Reduce(function(x, y) merge(x, y, all=TRUE),list(human_donors_CHLD2_FAMILY_table,CHLD2_perst_core_taxa_FAMILY_table_mice,CHLD2_perst_core_taxa_FAMILY_table_piglets))
View(merged_CHLD2_perst_coloniz_FAMILY)
write.table(merged_CHLD2_perst_coloniz_FAMILY, "CHLD2_perst_colonizer_taxa_breakdown_FAMILY.txt", sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)

human_donors_CHLD2_GENUS_table
merged_CHLD2_perst_coloniz_GENUS <- Reduce(function(x, y) merge(x, y, all=TRUE),list(human_donors_CHLD2_GENUS_table,CHLD2_perst_core_taxa_GENUS_table_mice,CHLD2_perst_core_taxa_GENUS_table_piglets))
View(merged_CHLD2_perst_coloniz_GENUS)
write.table(merged_CHLD2_perst_coloniz_GENUS, "CHLD2_perst_colonizer_taxa_breakdown_GENUS.txt", sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)
```
...
ADLT2_HMA_mice_ps
ADLT2_colonizers_mice_ps <- prune_taxa(perst_colonizing_core_ADLT2_mice,ADLT2_HMA_mice_ps)
ADLT2_colonizers_mice_ps

# at PHYLUM level
ADLT2_perst_core_taxa_PHYLUM <- table(tax_table(ADLT2_colonizers_mice_ps)[, "Phylum"], exclude = NULL)
...
colnames(ADLT2_perst_core_taxa_PHYLUM_table_mice)[colnames(ADLT2_perst_core_taxa_PHYLUM_table_mice) == "Freq"] <- "number of ASVs in mice"
...
# at FAMILY level
ADLT2_perst_core_taxa_FAMILY <- table(tax_table(ADLT2_colonizers_mice_ps)[, "Family"], exclude = NULL)
...
colnames(ADLT2_perst_core_taxa_FAMILY_table_mice)[colnames(ADLT2_perst_core_taxa_FAMILY_table_mice) == "Freq"] <- "number of ASVs in mice"
...
# at GENUS level
ADLT2_perst_core_taxa_GENUS <- table(tax_table(ADLT2_colonizers_mice_ps)[, "Genus"], exclude = NULL)
...
colnames(ADLT2_perst_core_taxa_GENUS_table_mice)[colnames(ADLT2_perst_core_taxa_GENUS_table_mice) == "Freq"] <- "number of ASVs in mice"
...
ADLT2_HMA_piglets_ps
ADLT2_colonizers_piglets_ps <- prune_taxa(perst_colonizing_core_ADLT2_piglets,ADLT2_HMA_piglets_ps)
ADLT2_colonizers_piglets_ps

# at PHYLUM level
ADLT2_perst_core_taxa_PHYLUM <- table(tax_table(ADLT2_colonizers_piglets_ps)[, "Phylum"], exclude = NULL)
...
colnames(ADLT2_perst_core_taxa_PHYLUM_table_piglets)[colnames(ADLT2_perst_core_taxa_PHYLUM_table_piglets) == "Freq"] <- "number of ASVs in piglets"
...
# at FAMILY level
ADLT2_perst_core_taxa_FAMILY <- table(tax_table(ADLT2_colonizers_piglets_ps)[, "Family"], exclude = NULL)
...
colnames(ADLT2_perst_core_taxa_FAMILY_table_piglets)[colnames(ADLT2_perst_core_taxa_FAMILY_table_piglets) == "Freq"] <- "number of ASVs in piglets"
...
# at GENUS level
ADLT2_perst_core_taxa_GENUS <- table(tax_table(ADLT2_colonizers_piglets_ps)[, "Genus"], exclude = NULL)
...
colnames(ADLT2_perst_core_taxa_GENUS_table_piglets)[colnames(ADLT2_perst_core_taxa_GENUS_table_piglets) == "Freq"] <- "number of ASVs in piglets"
...
# Merging info for all three species
human_donors_ADLT2_PHYLUM_table
merged_ADLT2_perst_coloniz_PHYLUM <- Reduce(function(x, y) merge(x, y, all=TRUE),list(human_donors_ADLT2_PHYLUM_table,ADLT2_perst_core_taxa_PHYLUM_table_mice,ADLT2_perst_core_taxa_PHYLUM_table_piglets))
View(merged_ADLT2_perst_coloniz_PHYLUM)
write.table(merged_ADLT2_perst_coloniz_PHYLUM, "ADLT2_perst_colonizer_taxa_breakdown_PHYLUM.txt", sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)

human_donors_ADLT2_FAMILY_table
merged_ADLT2_perst_coloniz_FAMILY <- Reduce(function(x, y) merge(x, y, all=TRUE),list(human_donors_ADLT2_FAMILY_table,ADLT2_perst_core_taxa_FAMILY_table_mice,ADLT2_perst_core_taxa_FAMILY_table_piglets))
View(merged_ADLT2_perst_coloniz_FAMILY)
write.table(merged_ADLT2_perst_coloniz_FAMILY, "ADLT2_perst_colonizer_taxa_breakdown_FAMILY.txt", sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)

human_donors_ADLT2_GENUS_table
merged_ADLT2_perst_coloniz_GENUS <- Reduce(function(x, y) merge(x, y, all=TRUE),list(human_donors_ADLT2_GENUS_table,ADLT2_perst_core_taxa_GENUS_table_mice,ADLT2_perst_core_taxa_GENUS_table_piglets))
View(merged_ADLT2_perst_coloniz_GENUS)
write.table(merged_ADLT2_perst_coloniz_GENUS, "ADLT2_perst_colonizer_taxa_breakdown_GENUS.txt", sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)
```
...
ELD2_HMA_mice_ps
ELD2_colonizers_mice_ps <- prune_taxa(perst_colonizing_core_ELD2_mice,ELD2_HMA_mice_ps)
ELD2_colonizers_mice_ps

# at PHYLUM level
ELD2_perst_core_taxa_PHYLUM <- table(tax_table(ELD2_colonizers_mice_ps)[, "Phylum"], exclude = NULL)
...
colnames(ELD2_perst_core_taxa_PHYLUM_table_mice)[colnames(ELD2_perst_core_taxa_PHYLUM_table_mice) == "Freq"] <- "number of ASVs in  mice"
...
# at FAMILY level
ELD2_perst_core_taxa_FAMILY <- table(tax_table(ELD2_colonizers_mice_ps)[, "Family"], exclude = NULL)
...
colnames(ELD2_perst_core_taxa_FAMILY_table_mice)[colnames(ELD2_perst_core_taxa_FAMILY_table_mice) == "Freq"] <- "number of ASVs in mice"
...
# at GENUS level
ELD2_perst_core_taxa_GENUS <- table(tax_table(ELD2_colonizers_mice_ps)[, "Genus"], exclude = NULL)
...
colnames(ELD2_perst_core_taxa_GENUS_table_mice)[colnames(ELD2_perst_core_taxa_GENUS_table_mice) == "Freq"] <- "number of ASVs in mice"
...
ELD2_HMA_piglets_ps
ELD2_colonizers_piglets_ps <- prune_taxa(perst_colonizing_core_ELD2_piglets,ELD2_HMA_piglets_ps)
ELD2_colonizers_piglets_ps

# at PHYLUM level
ELD2_perst_core_taxa_PHYLUM <- table(tax_table(ELD2_colonizers_piglets_ps)[, "Phylum"], exclude = NULL)
...
colnames(ELD2_perst_core_taxa_PHYLUM_table_piglets)[colnames(ELD2_perst_core_taxa_PHYLUM_table_piglets) == "Freq"] <- "number of ASVs in piglets"
...
# at FAMILY level
ELD2_perst_core_taxa_FAMILY <- table(tax_table(ELD2_colonizers_piglets_ps)[, "Family"], exclude = NULL)
...
colnames(ELD2_perst_core_taxa_FAMILY_table_piglets)[colnames(ELD2_perst_core_taxa_FAMILY_table_piglets) == "Freq"] <- "number of ASVs in piglets"
...
# at GENUS level
ELD2_perst_core_taxa_GENUS <- table(tax_table(ELD2_colonizers_piglets_ps)[, "Genus"], exclude = NULL)
...
colnames(ELD2_perst_core_taxa_GENUS_table_piglets)[colnames(ELD2_perst_core_taxa_GENUS_table_piglets) == "Freq"] <- "number of ASVs in piglets"
...
# Merging info for all three species
human_donors_ELD2_PHYLUM_table
merged_ELD2_perst_coloniz_PHYLUM <- Reduce(function(x, y) merge(x, y, all=TRUE),list(human_donors_ELD2_PHYLUM_table,ELD2_perst_core_taxa_PHYLUM_table_mice,ELD2_perst_core_taxa_PHYLUM_table_piglets))
View(merged_ELD2_perst_coloniz_PHYLUM)
write.table(merged_ELD2_perst_coloniz_PHYLUM, "ELD2_perst_colonizer_taxa_breakdown_PHYLUM.txt", sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)

human_donors_ELD2_FAMILY_table
merged_ELD2_perst_coloniz_FAMILY <- Reduce(function(x, y) merge(x, y, all=TRUE),list(human_donors_ELD2_FAMILY_table,ELD2_perst_core_taxa_FAMILY_table_mice,ELD2_perst_core_taxa_FAMILY_table_piglets))
View(merged_ELD2_perst_coloniz_FAMILY)
write.table(merged_ELD2_perst_coloniz_FAMILY, "ELD2_perst_colonizer_taxa_breakdown_FAMILY.txt", sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)

human_donors_ELD2_GENUS_table
merged_ELD2_perst_coloniz_GENUS <- Reduce(function(x, y) merge(x, y, all=TRUE),list(human_donors_ELD2_GENUS_table,ELD2_perst_core_taxa_GENUS_table_mice,ELD2_perst_core_taxa_GENUS_table_piglets))
View(merged_ELD2_perst_coloniz_GENUS)
write.table(merged_ELD2_perst_coloniz_GENUS, "ELD2_perst_colonizer_taxa_breakdown_GENUS.txt", sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)
```
...
**Peforming weekly alpha diversity comparisons for Donor_1(INF2)**
...
wilcox.test(donors, week_1) # not significant (ns)
wilcox.test(donors, week_2) # ns
wilcox.test(donors, week_3) # ns
wilcox.test(donors, week_4) # significant (p=0.018)
...
wilcox.test(two_days, week_4) # significant (p=0.020)
wilcox.test(two_days, week_5) # significant (p=0.029)
wilcox.test(two_days, final) # ns

# let's also compare alpha diversity between each time point for the mice
wilcox.test(two_days, week_1) # ns (p=0.38)
wilcox.test(week_1, week_2) # ns (p=0.08)
wilcox.test(week_2, week_3) # ns (p=1)
wilcox.test(week_3, week_4) # significant (p=0.03)
wilcox.test(week_4, week_5) # ns (p=0.09)
wilcox.test(week_5, final) # ns (p=0.32)
...
wilcox.test(donors, two_days) # significant (p=0.016)
wilcox.test(donors, week_1) # significant (p=0.036)
wilcox.test(donors, week_2) # significant (p=0.036)
wilcox.test(donors, week_3) # ns
wilcox.test(donors, week_4) # significant (p=0.036)
wilcox.test(donors, week_5) # ns
wilcox.test(donors, final)  # significant (p=0.036)
...
wilcox.test(two_days, week_1) # ns (p=0.4)
wilcox.test(week_1, week_2) # ns (p=1)
wilcox.test(week_2, week_3) # ns (p=1)
wilcox.test(week_3, week_4) # ns (p=1)
wilcox.test(week_4, week_5) # ns (p=1)
wilcox.test(week_5, final) # ns (p=0.2)
```


**Peforming weekly alpha diversity comparisons for Donor_2(CHLD2)**
...
**Peforming weekly alpha diversity comparisons for Donor_3(ADLT2)**
...
**Peforming weekly alpha diversity comparisons for Donor_4(ELD2/SNR2)**
...
We also thought that it would be useful to compare the relative abundances of the persistent colonizer ASVs to the HMA animal models at each time point
...
sigtab$ASV_ID <- rownames(sigtab) # adding ASV_ID as a column header
sigtab <- sigtab[, c(14,1:13)] # setting dataframe columns in the desired order
sigtab_perst_col <- sigtab[sigtab$ASV_ID %in% perst_colonizing_core_CHLD2_piglets,] # Now let's separate out the persistent colonizer ASV
View(sigtab_perst_col)
...
perst_colonizing_core_INF2_mice
perst_rel_abund_core_INF2_taxa_mice
coreoniz_and_abund_INF2_taxa_mice <- Reduce(intersect, list(perst_colonizing_core_INF2_mice, perst_rel_abund_core_INF2_taxa_mice))
coreoniz_and_abund_INF2_taxa_mice # 7 ASVs 


# for piglets
perst_colonizing_core_INF2_piglets
perst_rel_abund_core_INF2_taxa_piglets
coreoniz_and_abund_INF2_taxa_piglets <- Reduce(intersect, list(perst_colonizing_core_INF2_piglets, perst_rel_abund_core_INF2_taxa_piglets))
coreoniz_and_abund_INF2_taxa_piglets # 8 ASVs 

# commonly persistent and donor-like abundant in both animal models
common_coreoniz_and_abund_INF2_taxa <- Reduce(intersect,list(coreoniz_and_abund_INF2_taxa_mice, coreoniz_and_abund_INF2_taxa_piglets))
common_coreoniz_and_abund_INF2_taxa # 0 taxa
...
perst_colonizing_core_CHLD2_mice
perst_rel_abund_core_CHLD2_taxa_mice
coreoniz_and_abund_CHLD2_taxa_mice <- Reduce(intersect, list(perst_colonizing_core_CHLD2_mice, perst_rel_abund_core_CHLD2_taxa_mice))
coreoniz_and_abund_CHLD2_taxa_mice # 2 ASV (ASV_107 & ASV_55)

# for piglets
perst_colonizing_core_CHLD2_piglets
perst_rel_abund_core_CHLD2_taxa_piglets
coreoniz_and_abund_CHLD2_taxa_piglets <- Reduce(intersect, list(perst_colonizing_core_CHLD2_piglets, perst_rel_abund_core_CHLD2_taxa_piglets))
coreoniz_and_abund_CHLD2_taxa_piglets # 15 ASVs
 

# Let's look at the classification of these 15 ASVs
CHLD2_core_and_abund_ps <- prune_taxa(coreoniz_and_abund_CHLD2_taxa_piglets,CHLD2_HMA_piglets_ps)
CHLD2_core_and_abund_ps

# at FAMILY level
CHLD2_perst_and_abund_core_taxa_FAMILY <- table(tax_table(CHLD2_core_and_abund_ps)[, "Family"], exclude = NULL)
...
# at GENUS level
CHLD2_perst_and_abund_core_taxa_GENUS <- table(tax_table(CHLD2_core_and_abund_ps)[, "Genus"], exclude = NULL)
...
# commonly persistent and donor-like abundant in both animal models
common_coreoniz_and_abund_CHLD2_taxa <- Reduce(intersect,list(coreoniz_and_abund_CHLD2_taxa_mice, coreoniz_and_abund_CHLD2_taxa_piglets))
common_coreoniz_and_abund_CHLD2_taxa # 2 ASVs (ASV_107, ASV_55)
...
perst_colonizing_core_ADLT2_mice
perst_rel_abund_core_ADLT2_taxa_mice
coreoniz_and_abund_ADLT2_taxa_mice <- Reduce(intersect, list(perst_colonizing_core_ADLT2_mice, perst_rel_abund_core_ADLT2_taxa_mice))
coreoniz_and_abund_ADLT2_taxa_mice # 6 ASVs 

# for piglets
perst_colonizing_core_ADLT2_piglets
perst_rel_abund_core_ADLT2_taxa_piglets
coreoniz_and_abund_ADLT2_taxa_piglets <- Reduce(intersect, list(perst_colonizing_core_ADLT2_piglets, perst_rel_abund_core_ADLT2_taxa_piglets))
coreoniz_and_abund_ADLT2_taxa_piglets # 40 ASVs

# Let's look at the classification of these 40 ASVs
ADLT2_core_and_abund_ps <- prune_taxa(coreoniz_and_abund_ADLT2_taxa_piglets,ADLT2_HMA_piglets_ps)
ADLT2_core_and_abund_ps

# at FAMILY level
ADLT2_perst_and_abund_core_taxa_FAMILY <- table(tax_table(ADLT2_core_and_abund_ps)[, "Family"], exclude = NULL)
...
# at GENUS level
ADLT2_perst_and_abund_core_taxa_GENUS <- table(tax_table(ADLT2_core_and_abund_ps)[, "Genus"], exclude = NULL)
...
# commonly persistent and donor-like abundant in both animal models
common_coreoniz_and_abund_ADLT2_taxa <- Reduce(intersect,list(coreoniz_and_abund_ADLT2_taxa_mice, coreoniz_and_abund_ADLT2_taxa_piglets))
common_coreoniz_and_abund_ADLT2_taxa # Two ASVs (ASV_57, and ASV_187)
...
perst_colonizing_core_ELD2_mice
perst_rel_abund_core_ELD2_taxa_mice
coreoniz_and_abund_ELD2_taxa_mice <- Reduce(intersect, list(perst_colonizing_core_ELD2_mice, perst_rel_abund_core_ELD2_taxa_mice))
coreoniz_and_abund_ELD2_taxa_mice # 7 ASVs

# for piglets
perst_colonizing_core_ELD2_piglets
perst_rel_abund_core_ELD2_taxa_piglets
coreoniz_and_abund_ELD2_taxa_piglets <- Reduce(intersect, list(perst_colonizing_core_ELD2_piglets, perst_rel_abund_core_ELD2_taxa_piglets))
coreoniz_and_abund_ELD2_taxa_piglets # 16 ASVs

# Let's look at the classification of these 16 ASVs
ELD2_core_and_abund_ps <- prune_taxa(coreoniz_and_abund_ELD2_taxa_piglets,ELD2_HMA_piglets_ps)
ELD2_core_and_abund_ps

# at FAMILY level
ELD2_perst_and_abund_core_taxa_FAMILY <- table(tax_table(ELD2_core_and_abund_ps)[, "Family"], exclude = NULL)
...
# at GENUS level
ELD2_perst_and_abund_core_taxa_GENUS <- table(tax_table(ELD2_core_and_abund_ps)[, "Genus"], exclude = NULL)
...
# commonly persistent and donor-like abundant in both animal models
common_coreoniz_and_abund_ELD2_taxa <- Reduce(intersect,list(coreoniz_and_abund_ELD2_taxa_mice, coreoniz_and_abund_ELD2_taxa_piglets))
common_coreoniz_and_abund_ELD2_taxa # 2 common ASVs (ASV_86 and ASV_92)
```
...
sum(otu_table(overall_common_core_CHLD2_inocula_ps))/sum(otu_table(human_donors_CHLD2_inocula_core_ps)) # 55% of reads accounted for by the 'overall core'

common_overall_core_CHLD2_mice <- Reduce(intersect, list(common_core_ASVs_no_INF2,core_CHLD2_taxa_colonizing_mice)) # 9 ASVs
common_overall_core_CHLD2_piglets <- Reduce(intersect, list(common_core_ASVs_no_INF2,core_CHLD2_taxa_colonizing_piglets)) # 24 ASVs

CHLD2_HMA_mice_overall_core_ps <- prune_taxa(common_overall_core_CHLD2_mice, CHLD2_HMA_mice_core_ps)
sum(otu_table(CHLD2_HMA_mice_overall_core_ps))/sum(otu_table(CHLD2_HMA_mice_core_ps)) # 33.3% of reads

CHLD2_HMA_piglets_overall_core_ps <- prune_taxa(common_overall_core_CHLD2_piglets, CHLD2_HMA_piglets_core_ps)
sum(otu_table(CHLD2_HMA_piglets_overall_core_ps))/sum(otu_table(CHLD2_HMA_piglets_core_ps)) # 35.2% of reads

persistent_coloniz_core_27_CHLD2_mice <- Reduce(intersect, list(perst_colonizing_core_CHLD2_mice,common_core_ASVs_no_INF2)) 
persistent_coloniz_core_27_CHLD2_piglets <- Reduce(intersect, list(perst_colonizing_core_CHLD2_piglets,common_core_ASVs_no_INF2))
...
sum(otu_table(overall_common_core_ADLT2_inocula_ps))/sum(otu_table(human_donors_ADLT2_inocula_core_ps)) # 32% of reads accounted for by the 'overall core'

common_overall_core_ADLT2_mice <- Reduce(intersect, list(common_core_ASVs_no_INF2,core_ADLT2_taxa_colonizing_mice)) # 15 ASVs
common_overall_core_ADLT2_piglets <- Reduce(intersect, list(common_core_ASVs_no_INF2,core_ADLT2_taxa_colonizing_piglets)) # 25 ASVs

ADLT2_HMA_mice_overall_core_ps <- prune_taxa(common_overall_core_ADLT2_mice, ADLT2_HMA_mice_core_ps)
sum(otu_table(ADLT2_HMA_mice_overall_core_ps))/sum(otu_table(ADLT2_HMA_mice_core_ps)) # 45.2% of reads

ADLT2_HMA_piglets_overall_core_ps <- prune_taxa(common_overall_core_ADLT2_piglets, ADLT2_HMA_piglets_core_ps)
sum(otu_table(ADLT2_HMA_piglets_overall_core_ps))/sum(otu_table(ADLT2_HMA_piglets_core_ps)) # 31.3% of reads

persistent_coloniz_core_27_ADLT2_mice <- Reduce(intersect, list(perst_colonizing_core_ADLT2_mice,common_core_ASVs_no_INF2)) 
persistent_coloniz_core_27_ADLT2_piglets <- Reduce(intersect, list(perst_colonizing_core_ADLT2_piglets,common_core_ASVs_no_INF2))
...
sum(otu_table(overall_common_core_ELD2_inocula_ps))/sum(otu_table(human_donors_ELD2_inocula_core_ps)) # 27% of reads accounted for by the 'overall core'

common_overall_core_ELD2_mice <- Reduce(intersect, list(common_core_ASVs_no_INF2,core_ELD2_taxa_colonizing_mice)) # 10 ASVs
common_overall_core_ELD2_piglets <- Reduce(intersect, list(common_core_ASVs_no_INF2,core_ELD2_taxa_colonizing_piglets)) # 19 ASVs

ELD2_HMA_mice_overall_core_ps <- prune_taxa(common_overall_core_ELD2_mice, ELD2_HMA_mice_core_ps)
sum(otu_table(ELD2_HMA_mice_overall_core_ps))/sum(otu_table(ELD2_HMA_mice_core_ps)) # 20.9% of reads

ELD2_HMA_piglets_overall_core_ps <- prune_taxa(common_overall_core_ELD2_piglets, ELD2_HMA_piglets_core_ps)
sum(otu_table(ELD2_HMA_piglets_overall_core_ps))/sum(otu_table(ELD2_HMA_piglets_core_ps)) # 15.7.3% of reads

persistent_coloniz_core_27_ELD2_mice <- Reduce(intersect, list(perst_colonizing_core_ELD2_mice,common_core_ASVs_no_INF2)) 
persistent_coloniz_core_27_ELD2_piglets <- Reduce(intersect, list(perst_colonizing_core_ELD2_piglets,common_core_ASVs_no_INF2))
```
...
# Let's get the taxonomic classifications for these ASVs whcih didn't colonize either animal model
pig_mouse_comp_INF2_comparison_ps
...
highlight.sector(H_ASVs, track.index = 1, col = "red", text = "core ASVs of Donor_1", cex = 0.6, text.col = "white", niceFacing = TRUE, facing = "bending.inside")
highlight.sector(M_ASVs, track.index = 1, col = "dark green", text = "core Donor_1 ASVs colonizing  HMA mice ", cex = 0.55, text.col = "white", niceFacing = TRUE, facing = "bending.inside")
highlight.sector(P_ASVs, track.index = 1, col = "dark blue", text = "core Donor_1 ASVs colonizing HMA piglets", cex = 0.35, text.col = "white", niceFacing = TRUE, facing = "bending.inside")
...
# Setting the outer sectors 
highlight.sector(H_ASVs, track.index = 1, col = "red", text = "core ASVs of Donor_2", cex = 0.7, text.col = "white", niceFacing = TRUE, facing = "bending.inside")
highlight.sector(M_ASVs, track.index = 1, col = "dark green", text = "core Donor_2 ASVs colonizing HMA mice ", cex = 0.6, text.col = "white", niceFacing = TRUE, facing = "bending.inside")
highlight.sector(P_ASVs, track.index = 1, col = "dark blue", text = "core Donor_2 ASVs colonizing HMA piglets", cex = 0.6, text.col = "white", niceFacing = TRUE, facing = "bending.inside")
...
# Setting the outer sectors 
highlight.sector(H_ASVs, track.index = 1, col = "red", text = "core ASVs of Donor_3", cex = 0.7, text.col = "white", niceFacing = TRUE, facing = "bending.inside")
highlight.sector(M_ASVs, track.index = 1, col = "dark green", text = "core Donor_3 ASVs colonizing  HMA mice ", cex = 0.5, text.col = "white", niceFacing = TRUE, facing = "bending.inside")
highlight.sector(P_ASVs, track.index = 1, col = "dark blue", text = "core Donor_3 ASVs colonizing HMA piglets", cex = 0.6, text.col = "white", niceFacing = TRUE, facing = "bending.inside")
...
# Setting the outer sectors 
highlight.sector(H_ASVs, track.index = 1, col = "red", text = "core ASVs of Donor_4", cex = 0.7, text.col = "white", niceFacing = TRUE, facing = "bending.inside")
highlight.sector(M_ASVs, track.index = 1, col = "dark green", text = "core Donor_4 ASVs colonizing HMA mice ", cex = 0.55, text.col = "white", niceFacing = TRUE, facing = "bending.inside")
highlight.sector(P_ASVs, track.index = 1, col = "dark blue", text = "core Donor_4 ASVs colonizing HMA piglets", cex = 0.6, text.col = "white", niceFacing = TRUE, facing = "bending.inside")
...
# get sample data 
#sample_data_CHLD2 <- sample_data(rel_abund_CHLD2_core_ps) %>% select('SampleID','Species','Donor') %>% mutate_if(is.factor,as.character) ## For some reason (possibly due to a conflict with other packages) this is not working. So I'm doing teh following :
...
# get sample data 
#sample_data_CHLD2 <- sample_data(rel_abund_CHLD2_core_ps) %>% select('SampleID','Species','Donor') %>% mutate_if(is.factor,as.character)
...
# get sample data 
#sample_data_ADLT2 <- sample_data(rel_abund_ADLT2_core_ps) %>% select('SampleID','Species','Donor') %>% mutate_if(is.factor,as.character)
...
# get sample data 
#sample_data_ELD2 <- sample_data(rel_abund_ELD2_core_ps) %>% select('SampleID','Species','Donor') %>% mutate_if(is.factor,as.character)
...
merged_seqtab_all_runs_nochim <- removeBimeraDenovo(merged_seqtab_all_runs, method = "consensus", multithread = TRUE, verbose = TRUE)
dim(merged_seqtab_all_runs_nochim)
sum(merged_seqtab_all_runs_nochim)/sum(merged_seqtab_all_runs) 
...
merged_phyloseq_ps <- merge_phyloseq(otu_table(merged_seqtab_all_runs_nochim, taxa_are_rows = FALSE), sample_data(mapping_file), tax_table(taxa_table))
merged_phyloseq_ps
...
cyano_ps <- subset_taxa(merged_pig_mouse_comp_ps_master, Phylum == "Cyanobacteria")
cyano_ps # 5 cyanobacterial ASVs identified
...
pig_mouse_comp_PICRUSt_ps # we end up with a quality-filtered phyloseq object which has 588 ASVs
...
```{r, echo=TRUE}
picrust_biomfile <- "PICRUSt/INF/INF_KO_metagenome_out/INF_KO_pred_metagenome.biom" 
...
picrust_INF_ps.prop <- transform_sample_counts(picrust_INF_ps, function (OTU) OTU/sum(OTU)) # transform sample ciunts into relative abundances
picrust_INF_ps.prop 
...
core_threshold = nsamples(picrust_INF_donor_ps) # set threshold for the number of samples in which the ASV needs to be present in order for it to be considered as part of the core
...
bray_picrust_INF_core_KO <- plot_ordination(picrust_INF_core_KO_ps.prop, bray_ord_picrust_INF, color = "Species", shape = "Species") + ggtitle("PICRUSt INF PCoA plot") + theme(plot.title = element_text(hjust = 0.5, size = 10), legend.title = element_text(size = 10)) + labs(colour = "", shape = "host species")
...
core_threshold = nsamples(picrust_CHLD_donor_ps) # set threshold for the number of samples in which the ASV needs to be present in order for it to be considered as part of the core
...
bray_picrust_CHLD_core_KO <- plot_ordination(picrust_CHLD_core_KO_ps.prop, bray_ord_picrust_CHLD, color = "Species", shape = "Species") + ggtitle("PICRUSt CHLD PCoA plot") + theme(plot.title = element_text(hjust = 0.5, size = 10), legend.title = element_text(size = 10)) + labs(colour = "", shape = "host species")
...
# Let's read in the PICRUSt output, which is a function-based seqtab (which i converted to .biom format) to phyloseq
picrust_biomfile <- "PICRUSt/ADLT/ADLT_KO_pred_metagenome.biom" 
picrust_biom = biomformat::read_biom(biom_file = picrust_biomfile)
picrust_biom_table <- import_biom(picrust_biom) 
...
core_threshold = nsamples(picrust_ADLT_donor_ps) # set threshold for the number of samples in which the ASV needs to be present in order for it to be considered as part of the core
...
picrust_ADLT_core_KO_ps # 4411 core KOs identified

sum(otu_table(picrust_ADLT_core_KO_ps))/sum(otu_table(picrust_ADLT_ps)) # 99.64% of reads are retained
...
bray_picrust_ADLT_core_KO <- plot_ordination(picrust_ADLT_core_KO_ps.prop, bray_ord_picrust_ADLT, color = "Species", shape = "Species") + ggtitle("PICRUSt ADLT PCoA plot") + theme(plot.title = element_text(hjust = 0.5, size = 10), legend.title = element_text(size = 10)) + labs(colour = "", shape = "host species")
...
core_threshold = nsamples(picrust_ELD_donor_ps) # set threshold for the number of samples in which the ASV needs to be present in order for it to be considered as part of the core
...
bray_picrust_ELD_core_KO <- plot_ordination(picrust_ELD_core_KO_ps.prop, bray_ord_picrust_ELD, color = "Species", shape = "Species") + ggtitle("PICRUSt SNR PCoA plot") + theme(plot.title = element_text(hjust = 0.5, size = 10), legend.title = element_text(size = 10)) + labs(colour = "", shape = "host species")
...
The work below show the data analysis steps for the **supplementary materials** section of the study. This includes the follow-up comparison between HMA piglets and HMA mice for the establishment of a second infant donor microbiota as well as the evaluation of the establishment of donor taxa from a second set of CHLD, ADLT, and SNR donors in a different group of HMA mice (C3H/HeN).
...
Using the 'supp_mater_ASV.fna' FASTA file as the input file, a phylogenetic tree was made using the SILVA database and MOTHUR as described previously for the main material. Reading in the phylogenetic tree made in MOTHUR to R and phyloseq environment :
...
```{r, echo=TRUE}
mapping_file <- read.table('mapping_file_round_2.txt', sep = "\t", header = TRUE, comment.char = "")
row.names(mapping_file) <- as.character(mapping_file[,1])
View(mapping_file)
```
...
round_2_complete_ps <- readRDS("round_2_complete_ps.rds")
```
...
unw_ord_INF <- ordinate(round_2_INF_no_milk_ps.prop, method = "PCoA", distance = "unifrac", weighted = F)
unwunifrac_INF_round_2 <- plot_ordination(round_2_INF_no_milk_ps.prop, unw_ord_INF, color = "Species") + ggtitle("Infant donor PCoA plot - unweighted unifrac") + theme(plot.title = element_text(hjust = 0.5)) 
unwunifrac_INF_round_2

# plotting using Bray-Curtis distances
bray_ord_INF <- ordinate(round_2_INF_no_milk_ps.prop, method="PCoA", distance = "bray")
bray_pcoa_INF_round_2 <- plot_ordination(round_2_INF_no_milk_ps.prop, bray_ord_INF, color="Species") + ggtitle("Infant donor PCoA plot - Bray-Curtis") + theme(plot.title = element_text(hjust = 0.5))
...
bray_pcoa_INF_round_2_core <- plot_ordination(round_2_INF_no_milk_core_ps.prop, bray_ord_INF_core, color="Species", shape = "Species") + ggtitle("Infant donor core PCoA plot - Bray-Curtis") + theme(plot.title = element_text(hjust = 0.5)) + labs(colour = " ", shape = "host species")
...
human_donor_inocula_with_rel_taxa_ps

CHLD_round_2_donor_core_ps.prop <- transform_sample_counts(human_donor_inocula_with_rel_taxa_ps, function(x) x/sum(x))
...
CHLD_round_2_HMA_mice_donor_rel_taxa_ps

CHLD_round_2_mice_ps.prop <- transform_sample_counts(CHLD_round_2_HMA_mice_donor_rel_taxa_ps, function(x) x/sum(x))
CHLD_round_2_mice_ps.prop 
...
## at PHYLUM level for donors
human_donor_inocula_with_rel_taxa_ps # 117 core ASVs

ADLT_round_2_donor_core_ps.prop <- transform_sample_counts(human_donor_inocula_with_rel_taxa_ps, function(x) x/sum(x))
...
# at PHYLUM level for mice
ADLT_round_2_HMA_mice_donor_rel_taxa_ps

ADLT_round_2_mice_ps.prop <- transform_sample_counts(ADLT_round_2_HMA_mice_donor_rel_taxa_ps, function(x) x/sum(x))
ADLT_round_2_mice_ps.prop 
...
## at PHYLUM level for donors
human_donor_inocula_with_rel_taxa_ps
SNR_round_2_donor_core_ps.prop <- transform_sample_counts(human_donor_inocula_with_rel_taxa_ps, function(x) x/sum(x))
...
# at PHYLUM level for mice
SNR_round_2_HMA_mice_donor_rel_taxa_ps

SNR_round_2_mice_ps.prop <- transform_sample_counts(SNR_round_2_HMA_mice_donor_rel_taxa_ps, function(x) x/sum(x))
SNR_round_2_mice_ps.prop 
...
#########################################################################################################
Trying out different analyses to look at colonization efficiency
#########################################################################################################


##**Looking at mice based on gender**##
...
### MICE divided in to 'Male' and 'Female' groups :

## Male mice
INF2_HMA_mice_core_male_ps <- subset_samples(INF2_HMA_mice_core_ps, Gender == 'male')
INF2_HMA_mice_core_male_ps

# Let's see how many core INF2 donor ASVs were found in at least one INF2 HMA male mouse fecal sample 
prevdf_INF2_core_male_mice <- makePrevDF(INF2_HMA_mice_core_male_ps, 1)
prevdf_INF2_core_male_mice_table <- as.data.frame(prevdf_INF2_core_male_mice[1])
colonized_prevdf_INF2_core_male_mice <- prevdf_INF2_core_male_mice_table[prevdf_INF2_core_male_mice_table$Prevalence >= 1,]
View(colonized_prevdf_INF2_core_male_mice) # 21 of the 26 core INF2 donor ASVs found in at least one INF2 mouse fecal sample. These are the 'COLONIZERS'
core_INF2_taxa_colonizing_male_mice <-prevdf_INF2_core_male_mice[2]
core_INF2_taxa_colonizing_male_mice <- unlist(core_INF2_taxa_colonizing_male_mice) 
core_INF2_taxa_colonizing_male_mice

# split by description to get weekly .....
# Prevalence of ASVs at 48 HRS
INF2_HMA_male_mice_core_ps_48HRS <- subset_samples(INF2_HMA_mice_core_male_ps, Description == '48hrs_post_inoc')
INF2_HMA_male_mice_core_ps_48HRS

prevdf_48HRS <- makePrevDF(INF2_HMA_male_mice_core_ps_48HRS,2) 
View(prevdf_48HRS)
prevdf_48HRS_table <- as.data.frame(prevdf_48HRS[1])
View(prevdf_48HRS_table)
colonized_prevdf_48HRS <- prevdf_48HRS_table[prevdf_48HRS_table$Prevalence >= 2,]
...
# Prevalence of ASVs at 1 week
INF2_HMA_male_mice_core_ps_1week <- subset_samples(INF2_HMA_mice_core_male_ps, Description == '1week_post_inoc')
INF2_HMA_male_mice_core_ps_1week

prevdf_1week <- makePrevDF(INF2_HMA_male_mice_core_ps_1week,2)
prevdf_1week_table <- as.data.frame(prevdf_1week[1])
colonized_prevdf_1week <- prevdf_1week_table[prevdf_1week_table$Prevalence >= 2,]
colonized_prevdf_1week$ASV_ID <- rownames(colonized_prevdf_1week)
colonized_prevdf_1week <- colonized_prevdf_1week[,c(3,1,2)]
...
# Prevalence of ASVs at 2 weeks
INF2_HMA_male_mice_core_ps_2weeks <- subset_samples(INF2_HMA_mice_core_male_ps, Description == '2weeks_post_inoc')
INF2_HMA_male_mice_core_ps_2weeks

prevdf_2weeks <- makePrevDF(INF2_HMA_male_mice_core_ps_2weeks,2)
prevdf_2weeks_table <- as.data.frame(prevdf_2weeks[1])
View(prevdf_2weeks_table)
colonized_prevdf_2weeks <- prevdf_2weeks_table[prevdf_2weeks_table$Prevalence >= 2,]
colonized_prevdf_2weeks$ASV_ID <- rownames(colonized_prevdf_2weeks)
...
# Prevalence of ASVs at 3 weeks
INF2_HMA_male_mice_core_ps_3weeks <- subset_samples(INF2_HMA_mice_core_male_ps, Description == '3weeks_post_inoc')
INF2_HMA_male_mice_core_ps_3weeks

prevdf_3weeks <- makePrevDF(INF2_HMA_male_mice_core_ps_3weeks,2)
prevdf_3weeks_table <- as.data.frame(prevdf_3weeks[1])
colonized_prevdf_3weeks <- prevdf_3weeks_table[prevdf_3weeks_table$Prevalence >= 2,]
colonized_prevdf_3weeks$ASV_ID <- rownames(colonized_prevdf_3weeks)
colonized_prevdf_3weeks <- colonized_prevdf_3weeks[,c(3,1,2)]
...
# Prevalence of ASVs at 4 weeks
INF2_HMA_male_mice_core_ps_4weeks <- subset_samples(INF2_HMA_mice_core_male_ps, Description == '4weeks_post_inoc')
INF2_HMA_male_mice_core_ps_4weeks

prevdf_4weeks <- makePrevDF(INF2_HMA_male_mice_core_ps_4weeks,2)
prevdf_4weeks_table <- as.data.frame(prevdf_4weeks[1])
View(prevdf_4weeks_table)
colonized_prevdf_4weeks <- prevdf_4weeks_table[prevdf_4weeks_table$Prevalence >= 2,]
colonized_prevdf_4weeks$ASV_ID <- rownames(colonized_prevdf_4weeks)
colonized_prevdf_4weeks <- colonized_prevdf_4weeks[,c(3,1,2)]
tp_list <- rep('1', (nrow(colonized_prevdf_4weeks)))
...
# Prevalence of ASVs at 5 weeks
INF2_HMA_male_mice_core_ps_5weeks <- subset_samples(INF2_HMA_mice_core_male_ps, Description == '5weeks_post_inoc')
INF2_HMA_male_mice_core_ps_5weeks

prevdf_5weeks <- makePrevDF(INF2_HMA_male_mice_core_ps_5weeks,2)
prevdf_5weeks_table <- as.data.frame(prevdf_5weeks[1])
colonized_prevdf_5weeks <- prevdf_5weeks_table[prevdf_5weeks_table$Prevalence >= 2,]
colonized_prevdf_5weeks$ASV_ID <- rownames(colonized_prevdf_5weeks)
colonized_prevdf_5weeks <- colonized_prevdf_5weeks[,c(3,1,2)]
...
# Prevalence of ASVs at 40 days
INF2_HMA_male_mice_core_ps_40days <- subset_samples(INF2_HMA_mice_core_male_ps, Description == 'Final_collection')
INF2_HMA_male_mice_core_ps_40days

prevdf_40days <- makePrevDF(INF2_HMA_male_mice_core_ps_40days,2)
prevdf_40days_table <- as.data.frame(prevdf_40days[1])
colonized_prevdf_40days <- prevdf_40days_table[prevdf_40days_table$Prevalence >= 2,]
colonized_prevdf_40days$ASV_ID <- rownames(colonized_prevdf_40days)
colonized_prevdf_40days <- colonized_prevdf_40days[,c(3,1,2)]
...
# Merging all dataframes together
merged_prev_df_INF2_core_male_mice <- Reduce(function(x, y) merge(x, y, all=TRUE), list(colonized_prevdf_48HRS_relv, colonized_prevdf_1week_relv, colonized_prevdf_2weeks_relv, colonized_prevdf_3weeks_relv, colonized_prevdf_4weeks_relv, colonized_prevdf_5weeks_relv, colonized_prevdf_40days_relv))
str(merged_prev_df_INF2_core_male_mice) # All columns considered factors
merged_prev_df_INF2_core_male_mice[2:8] <- lapply(merged_prev_df_INF2_core_male_mice[2:8], as.numeric) # need to convert columns 2-8 to numeric in order to do 'rowSums'
str(merged_prev_df_INF2_core_male_mice)
View(merged_prev_df_INF2_core_male_mice)
     
merged_prev_df_INF2_core_male_mice[is.na(merged_prev_df_INF2_core_male_mice)] <- 0 # Have to replace 'NA' (missing data) with '0' in order for 'rowSums' to work properly
merged_prev_df_INF2_core_male_mice$total <- rowSums(merged_prev_df_INF2_core_male_mice[2:8]) # add the values in the rows and use them to populate a new column called 'total'
View(merged_prev_df_INF2_core_male_mice)
merged_prev_df_INF2_core_male_mice$ASV_ID <- as.character(as.factor(merged_prev_df_INF2_core_male_mice$ASV_ID)) # convert 'ASV_ID' column to a character vector
str(merged_prev_df_INF2_core_male_mice)
write.table(merged_prev_df_INF2_core_male_mice, "merged_prev_df_INF2_core_male_mice.txt", sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)
...
temporally_colonizing_core_taxa_INF2_male_mice <- merged_prev_df_INF2_core_male_mice[,1]
temporally_colonizing_core_taxa_INF2_male_mice # 19 core ASVs in male mice

# actually colonizing taxa defined as taxa found in 2/3 of animals in at least 3 time points
actually_colonizing_core_taxa_INF2_male_mice_df <- subset(merged_prev_df_INF2_core_male_mice, total >= '3') # select the ASVs which were detected in at least 3 time points
View(actually_colonizing_core_taxa_INF2_male_mice_df)
actually_colonizing_core_taxa_INF2_male_mice <- actually_colonizing_core_taxa_INF2_male_mice_df[,1]
actually_colonizing_core_taxa_INF2_male_mice # 16 ASVs colonizing male mice

# persistent colonizers defined as taxa detected in all >= 4 time points
perst_colonizing_core_INF2_male_mice_df <- subset(merged_prev_df_INF2_core_male_mice, total >= '4') # select the ASVs which were detected in at least 4 time points
View(perst_colonizing_core_INF2_male_mice_df)
perst_colonizing_core_INF2_male_mice <- perst_colonizing_core_INF2_male_mice_df[,1]
perst_colonizing_core_INF2_male_mice # 14 ASVs persistently colonizing male mice. These are the 'PERSISTENT COLONIZERS'.

# true colonizers defined as taxa detected in all >= 5 time points
truly_colonizing_core_taxa_INF2_male_mice_df <- subset(merged_prev_df_INF2_core_male_mice, total >= '5') # select the ASVs which were detected in at least 5 time points
View(truly_colonizing_core_taxa_INF2_male_mice_df)
truly_colonizing_core_taxa_INF2_male_mice <- truly_colonizing_core_taxa_INF2_male_mice_df[,1]
truly_colonizing_core_taxa_INF2_male_mice # 14 ASVs 'truly' colonizing male mice


## Female mice
INF2_HMA_mice_core_female_ps <- subset_samples(INF2_HMA_mice_core_ps, Gender == 'female')
INF2_HMA_mice_core_female_ps

# Let's see how many core INF2 donor ASVs were found in at least one INF2 HMA female mouse fecal sample 
prevdf_INF2_core_female_mice <- makePrevDF(INF2_HMA_mice_core_female_ps, 1)
prevdf_INF2_core_female_mice_table <- as.data.frame(prevdf_INF2_core_female_mice[1])
colonized_prevdf_INF2_core_female_mice <- prevdf_INF2_core_female_mice_table[prevdf_INF2_core_female_mice_table$Prevalence >= 1,]
View(colonized_prevdf_INF2_core_female_mice) # 21 of the 26 core INF2 donor ASVs found in at least one INF2 mouse fecal sample. These are the 'COLONIZERS'
core_INF2_taxa_colonizing_female_mice <-prevdf_INF2_core_female_mice[2]
core_INF2_taxa_colonizing_female_mice <- unlist(core_INF2_taxa_colonizing_female_mice) 
core_INF2_taxa_colonizing_female_mice

# split by description to get weekly .....
# Prevalence of ASVs at 48 HRS
INF2_HMA_female_mice_core_ps_48HRS <- subset_samples(INF2_HMA_mice_core_female_ps, Description == '48hrs_post_inoc')
INF2_HMA_female_mice_core_ps_48HRS

prevdf_48HRS <- makePrevDF(INF2_HMA_female_mice_core_ps_48HRS,3) 
prevdf_48HRS_table <- as.data.frame(prevdf_48HRS[1])
View(prevdf_48HRS_table)
colonized_prevdf_48HRS <- prevdf_48HRS_table[prevdf_48HRS_table$Prevalence >= 3,]
colonized_prevdf_48HRS$ASV_ID <- rownames(colonized_prevdf_48HRS)
...
# Prevalence of ASVs at 1 week
INF2_HMA_female_mice_core_ps_1week <- subset_samples(INF2_HMA_mice_core_female_ps, Description == '1week_post_inoc')
INF2_HMA_female_mice_core_ps_1week

prevdf_1week <- makePrevDF(INF2_HMA_female_mice_core_ps_1week,3)
prevdf_1week_table <- as.data.frame(prevdf_1week[1])
colonized_prevdf_1week <- prevdf_1week_table[prevdf_1week_table$Prevalence >= 3,]
colonized_prevdf_1week$ASV_ID <- rownames(colonized_prevdf_1week)
colonized_prevdf_1week <- colonized_prevdf_1week[,c(3,1,2)]
...
# Prevalence of ASVs at 2 weeks
INF2_HMA_female_mice_core_ps_2weeks <- subset_samples(INF2_HMA_mice_core_female_ps, Description == '2weeks_post_inoc')
INF2_HMA_female_mice_core_ps_2weeks

prevdf_2weeks <- makePrevDF(INF2_HMA_female_mice_core_ps_2weeks,3)
prevdf_2weeks_table <- as.data.frame(prevdf_2weeks[1])
View(prevdf_2weeks_table)
colonized_prevdf_2weeks <- prevdf_2weeks_table[prevdf_2weeks_table$Prevalence >= 3,]
colonized_prevdf_2weeks$ASV_ID <- rownames(colonized_prevdf_2weeks)
colonized_prevdf_2weeks <- colonized_prevdf_2weeks[,c(3,1,2)]
tp_list <- rep('1', (nrow(colonized_prevdf_2weeks)))
colonized_prevdf_2weeks_relv <- data.frame(ASV_ID = rownames(colonized_prevdf_2weeks), Week_2 =  tp_list)
...
# Prevalence of ASVs at 3 weeks
INF2_HMA_female_mice_core_ps_3weeks <- subset_samples(INF2_HMA_mice_core_female_ps, Description == '3weeks_post_inoc')
INF2_HMA_female_mice_core_ps_3weeks

prevdf_3weeks <- makePrevDF(INF2_HMA_female_mice_core_ps_3weeks,3)
prevdf_3weeks_table <- as.data.frame(prevdf_3weeks[1])
...
# Prevalence of ASVs at 4 weeks
INF2_HMA_female_mice_core_ps_4weeks <- subset_samples(INF2_HMA_mice_core_female_ps, Description == '4weeks_post_inoc')
INF2_HMA_female_mice_core_ps_4weeks

prevdf_4weeks <- makePrevDF(INF2_HMA_female_mice_core_ps_4weeks,3)
prevdf_4weeks_table <- as.data.frame(prevdf_4weeks[1])
View(prevdf_4weeks_table)
colonized_prevdf_4weeks <- prevdf_4weeks_table[prevdf_4weeks_table$Prevalence >= 3,]
colonized_prevdf_4weeks$ASV_ID <- rownames(colonized_prevdf_4weeks)
colonized_prevdf_4weeks <- colonized_prevdf_4weeks[,c(3,1,2)]
tp_list <- rep('1', (nrow(colonized_prevdf_4weeks)))
colonized_prevdf_4weeks_relv <- data.frame(ASV_ID = rownames(colonized_prevdf_4weeks), Week_4 =  tp_list)
...
# Prevalence of ASVs at 5 weeks
INF2_HMA_female_mice_core_ps_5weeks <- subset_samples(INF2_HMA_mice_core_female_ps, Description == '5weeks_post_inoc')
INF2_HMA_female_mice_core_ps_5weeks

prevdf_5weeks <- makePrevDF(INF2_HMA_female_mice_core_ps_5weeks,3)
prevdf_5weeks_table <- as.data.frame(prevdf_5weeks[1])
colonized_prevdf_5weeks <- prevdf_5weeks_table[prevdf_5weeks_table$Prevalence >= 3,]
colonized_prevdf_5weeks$ASV_ID <- rownames(colonized_prevdf_5weeks)
colonized_prevdf_5weeks <- colonized_prevdf_5weeks[,c(3,1,2)]
...
# Prevalence of ASVs at 40 days
INF2_HMA_female_mice_core_ps_40days <- subset_samples(INF2_HMA_mice_core_female_ps, Description == 'Final_collection')
INF2_HMA_female_mice_core_ps_40days

prevdf_40days <- makePrevDF(INF2_HMA_female_mice_core_ps_40days,3)
prevdf_40days_table <- as.data.frame(prevdf_40days[1])
colonized_prevdf_40days <- prevdf_40days_table[prevdf_40days_table$Prevalence >= 3,]
colonized_prevdf_40days$ASV_ID <- rownames(colonized_prevdf_40days)
colonized_prevdf_40days <- colonized_prevdf_40days[,c(3,1,2)]
...
# Merging all dataframes together
merged_prev_df_INF2_core_female_mice <- Reduce(function(x, y) merge(x, y, all=TRUE), list(colonized_prevdf_48HRS_relv, colonized_prevdf_1week_relv, colonized_prevdf_2weeks_relv, colonized_prevdf_3weeks_relv, colonized_prevdf_4weeks_relv, colonized_prevdf_5weeks_relv, colonized_prevdf_40days_relv))
str(merged_prev_df_INF2_core_female_mice) # All columns considered factors
merged_prev_df_INF2_core_female_mice[2:8] <- lapply(merged_prev_df_INF2_core_female_mice[2:8], as.numeric) # need to convert columns 2-8 to numeric in order to do 'rowSums'
str(merged_prev_df_INF2_core_female_mice)
View(merged_prev_df_INF2_core_female_mice)
     
merged_prev_df_INF2_core_female_mice[is.na(merged_prev_df_INF2_core_female_mice)] <- 0 # Have to replace 'NA' (missing data) with '0' in order for 'rowSums' to work properly
merged_prev_df_INF2_core_female_mice$total <- rowSums(merged_prev_df_INF2_core_female_mice[2:8]) # add the values in the rows and use them to populate a new column called 'total'
View(merged_prev_df_INF2_core_female_mice)
merged_prev_df_INF2_core_female_mice$ASV_ID <- as.character(as.factor(merged_prev_df_INF2_core_female_mice$ASV_ID)) # convert 'ASV_ID' column to a character vector
str(merged_prev_df_INF2_core_female_mice)
write.table(merged_prev_df_INF2_core_female_mice, "merged_prev_df_INF2_core_female_mice.txt", sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)

# temporally colonizing taxa defined as taxa found in 2/3 of animals in at least 1 time point (these are all the ASVs included under the 'ASV_ID' column)
temporally_colonizing_core_taxa_INF2_female_mice <- merged_prev_df_INF2_core_female_mice[,1]
temporally_colonizing_core_taxa_INF2_female_mice # 20 core ASVs in female mice

# actually colonizing taxa defined as taxa found in 2/3 of animals in at least 3 time points
actually_colonizing_core_taxa_INF2_female_mice_df <- subset(merged_prev_df_INF2_core_female_mice, total >= '3') # select the ASVs which were detected in at least 3 time points
View(actually_colonizing_core_taxa_INF2_female_mice_df)
actually_colonizing_core_taxa_INF2_female_mice <- actually_colonizing_core_taxa_INF2_female_mice_df[,1]
actually_colonizing_core_taxa_INF2_female_mice # 19 ASVs colonizing mice

# persistent colonizers defined as taxa detected in all >= 4 time points
perst_colonizing_core_INF2_female_mice_df <- subset(merged_prev_df_INF2_core_female_mice, total >= '4') # select the ASVs which were detected in at least 4 time points
View(perst_colonizing_core_INF2_female_mice_df)
perst_colonizing_core_INF2_female_mice <- perst_colonizing_core_INF2_female_mice_df[,1]
str(perst_colonizing_core_INF2_female_mice) # 16 ASVs persistently colonizing mice. These are the 'PERSISTENT COLONIZERS'.

# true colonizers defined as taxa detected in all >= 5 time points
truly_colonizing_core_taxa_INF2_female_mice_df <- subset(merged_prev_df_INF2_core_female_mice, total >= '5') # select the ASVs which were detected in at least 5 time points
View(truly_colonizing_core_taxa_INF2_female_mice_df)
truly_colonizing_core_taxa_INF2_female_mice <- truly_colonizing_core_taxa_INF2_female_mice_df[,1]
truly_colonizing_core_taxa_INF2_female_mice # 15 ASVs 'truly' colonizing mice


core_INF2_persisters_in_both_genders_of_mice <- Reduce(intersect, list(perst_colonizing_core_INF2_female_mice ,perst_colonizing_core_INF2_male_mice))
core_INF2_persisters_in_both_genders_of_mice
...
## Male mice
CHLD2_HMA_mice_core_male_ps <- subset_samples(CHLD2_HMA_mice_core_ps, Gender == 'male')
CHLD2_HMA_mice_core_male_ps

# Let's see how many core CHLD2 donor ASVs were found in at least one CHLD2 HMA male mouse fecal sample 
prevdf_CHLD2_core_male_mice <- makePrevDF(CHLD2_HMA_mice_core_male_ps, 1)
prevdf_CHLD2_core_male_mice_table <- as.data.frame(prevdf_CHLD2_core_male_mice[1])
colonized_prevdf_CHLD2_core_male_mice <- prevdf_CHLD2_core_male_mice_table[prevdf_CHLD2_core_male_mice_table$Prevalence >= 1,]
View(colonized_prevdf_CHLD2_core_male_mice) # 32 of the 76 core CHLD2 donor ASVs found in at least one CHLD2 mouse fecal sample. These are the 'COLONIZERS'
core_CHLD2_taxa_colonizing_male_mice <-prevdf_CHLD2_core_male_mice[2]
core_CHLD2_taxa_colonizing_male_mice <- unlist(core_CHLD2_taxa_colonizing_male_mice) 
core_CHLD2_taxa_colonizing_male_mice

# split by description to get weekly .....
# Prevalence of ASVs at 48 HRS
CHLD2_HMA_male_mice_core_ps_48HRS <- subset_samples(CHLD2_HMA_mice_core_male_ps, Description == '48hrs_post_inoc')
CHLD2_HMA_male_mice_core_ps_48HRS

prevdf_48HRS <- makePrevDF(CHLD2_HMA_male_mice_core_ps_48HRS,3) 
View(prevdf_48HRS)
prevdf_48HRS_table <- as.data.frame(prevdf_48HRS[1])
View(prevdf_48HRS_table)
colonized_prevdf_48HRS <- prevdf_48HRS_table[prevdf_48HRS_table$Prevalence >= 3,]
...
# Prevalence of ASVs at 1 week
CHLD2_HMA_male_mice_core_ps_1week <- subset_samples(CHLD2_HMA_mice_core_male_ps, Description == '1week_post_inoc')
CHLD2_HMA_male_mice_core_ps_1week

prevdf_1week <- makePrevDF(CHLD2_HMA_male_mice_core_ps_1week,3)
prevdf_1week_table <- as.data.frame(prevdf_1week[1])
colonized_prevdf_1week <- prevdf_1week_table[prevdf_1week_table$Prevalence >= 3,]
colonized_prevdf_1week$ASV_ID <- rownames(colonized_prevdf_1week)
colonized_prevdf_1week <- colonized_prevdf_1week[,c(3,1,2)]
...
# Prevalence of ASVs at 2 weeks
CHLD2_HMA_male_mice_core_ps_2weeks <- subset_samples(CHLD2_HMA_mice_core_male_ps, Description == '2weeks_post_inoc')
CHLD2_HMA_male_mice_core_ps_2weeks

prevdf_2weeks <- makePrevDF(CHLD2_HMA_male_mice_core_ps_2weeks,3)
prevdf_2weeks_table <- as.data.frame(prevdf_2weeks[1])
View(prevdf_2weeks_table)
colonized_prevdf_2weeks <- prevdf_2weeks_table[prevdf_2weeks_table$Prevalence >= 3,]
colonized_prevdf_2weeks$ASV_ID <- rownames(colonized_prevdf_2weeks)
...
# Prevalence of ASVs at 3 weeks
CHLD2_HMA_male_mice_core_ps_3weeks <- subset_samples(CHLD2_HMA_mice_core_male_ps, Description == '3weeks_post_inoc')
CHLD2_HMA_male_mice_core_ps_3weeks

prevdf_3weeks <- makePrevDF(CHLD2_HMA_male_mice_core_ps_3weeks,3)
prevdf_3weeks_table <- as.data.frame(prevdf_3weeks[1])
colonized_prevdf_3weeks <- prevdf_3weeks_table[prevdf_3weeks_table$Prevalence >= 3,]
colonized_prevdf_3weeks$ASV_ID <- rownames(colonized_prevdf_3weeks)
colonized_prevdf_3weeks <- colonized_prevdf_3weeks[,c(3,1,2)]
...
# Prevalence of ASVs at 4 weeks
CHLD2_HMA_male_mice_core_ps_4weeks <- subset_samples(CHLD2_HMA_mice_core_male_ps, Description == '4weeks_post_inoc')
CHLD2_HMA_male_mice_core_ps_4weeks

prevdf_4weeks <- makePrevDF(CHLD2_HMA_male_mice_core_ps_4weeks,3)
prevdf_4weeks_table <- as.data.frame(prevdf_4weeks[1])
View(prevdf_4weeks_table)
colonized_prevdf_4weeks <- prevdf_4weeks_table[prevdf_4weeks_table$Prevalence >= 3,]
colonized_prevdf_4weeks$ASV_ID <- rownames(colonized_prevdf_4weeks)
...
# Prevalence of ASVs at 5 weeks
CHLD2_HMA_male_mice_core_ps_5weeks <- subset_samples(CHLD2_HMA_mice_core_male_ps, Description == '5weeks_post_inoc')
CHLD2_HMA_male_mice_core_ps_5weeks

prevdf_5weeks <- makePrevDF(CHLD2_HMA_male_mice_core_ps_5weeks,3)
prevdf_5weeks_table <- as.data.frame(prevdf_5weeks[1])
colonized_prevdf_5weeks <- prevdf_5weeks_table[prevdf_5weeks_table$Prevalence >= 3,]
colonized_prevdf_5weeks$ASV_ID <- rownames(colonized_prevdf_5weeks)
colonized_prevdf_5weeks <- colonized_prevdf_5weeks[,c(3,1,2)]
...
# Prevalence of ASVs at 40 days
CHLD2_HMA_male_mice_core_ps_40days <- subset_samples(CHLD2_HMA_mice_core_male_ps, Description == 'Final_collection')
CHLD2_HMA_male_mice_core_ps_40days

prevdf_40days <- makePrevDF(CHLD2_HMA_male_mice_core_ps_40days,3)
prevdf_40days_table <- as.data.frame(prevdf_40days[1])
colonized_prevdf_40days <- prevdf_40days_table[prevdf_40days_table$Prevalence >= 3,]
colonized_prevdf_40days$ASV_ID <- rownames(colonized_prevdf_40days)
colonized_prevdf_40days <- colonized_prevdf_40days[,c(3,1,2)]
...
# Merging all dataframes together
merged_prev_df_CHLD2_core_male_mice <- Reduce(function(x, y) merge(x, y, all=TRUE), list(colonized_prevdf_48HRS_relv, colonized_prevdf_1week_relv, colonized_prevdf_2weeks_relv, colonized_prevdf_3weeks_relv, colonized_prevdf_4weeks_relv, colonized_prevdf_5weeks_relv, colonized_prevdf_40days_relv))
str(merged_prev_df_CHLD2_core_male_mice) # All columns considered factors
merged_prev_df_CHLD2_core_male_mice[2:8] <- lapply(merged_prev_df_CHLD2_core_male_mice[2:8], as.numeric) # need to convert columns 2-8 to numeric in order to do 'rowSums'
str(merged_prev_df_CHLD2_core_male_mice)
View(merged_prev_df_CHLD2_core_male_mice)
     
merged_prev_df_CHLD2_core_male_mice[is.na(merged_prev_df_CHLD2_core_male_mice)] <- 0 # Have to replace 'NA' (missing data) with '0' in order for 'rowSums' to work properly
merged_prev_df_CHLD2_core_male_mice$total <- rowSums(merged_prev_df_CHLD2_core_male_mice[2:8]) # add the values in the rows and use them to populate a new column called 'total'
View(merged_prev_df_CHLD2_core_male_mice)
merged_prev_df_CHLD2_core_male_mice$ASV_ID <- as.character(as.factor(merged_prev_df_CHLD2_core_male_mice$ASV_ID)) # convert 'ASV_ID' column to a character vector
str(merged_prev_df_CHLD2_core_male_mice)
write.table(merged_prev_df_CHLD2_core_male_mice, "merged_prev_df_CHLD2_core_male_mice.txt", sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)

# temporally colonizing taxa defined as taxa found in 2/3 of animals in at least 1 time point (these are all the ASVs included under the 'ASV_ID' column)
temporally_colonizing_core_taxa_CHLD2_male_mice <- merged_prev_df_CHLD2_core_male_mice[,1]
temporally_colonizing_core_taxa_CHLD2_male_mice # 29 core ASVs in male mice

# actually colonizing taxa defined as taxa found in 2/3 of animals in at least 3 time points
actually_colonizing_core_taxa_CHLD2_male_mice_df <- subset(merged_prev_df_CHLD2_core_male_mice, total >= '3') # select the ASVs which were detected in at least 3 time points
View(actually_colonizing_core_taxa_CHLD2_male_mice_df)
actually_colonizing_core_taxa_CHLD2_male_mice <- actually_colonizing_core_taxa_CHLD2_male_mice_df[,1]
actually_colonizing_core_taxa_CHLD2_male_mice # 25 ASVs colonizing male mice

# persistent colonizers defined as taxa detected in all >= 4 time points
perst_colonizing_core_CHLD2_male_mice_df <- subset(merged_prev_df_CHLD2_core_male_mice, total >= '4') # select the ASVs which were detected in at least 4 time points
View(perst_colonizing_core_CHLD2_male_mice_df)
perst_colonizing_core_CHLD2_male_mice <- perst_colonizing_core_CHLD2_male_mice_df[,1]
perst_colonizing_core_CHLD2_male_mice # 22 ASVs persistently colonizing male mice. These are the 'PERSISTENT COLONIZERS'.

# true colonizers defined as taxa detected in all >= 5 time points
truly_colonizing_core_taxa_CHLD2_male_mice_df <- subset(merged_prev_df_CHLD2_core_male_mice, total >= '5') # select the ASVs which were detected in at least 5 time points
View(truly_colonizing_core_taxa_CHLD2_male_mice_df)
truly_colonizing_core_taxa_CHLD2_male_mice <- truly_colonizing_core_taxa_CHLD2_male_mice_df[,1]
truly_colonizing_core_taxa_CHLD2_male_mice # 21 ASVs 'truly' colonizing male mice


## Female mice
CHLD2_HMA_mice_core_female_ps <- subset_samples(CHLD2_HMA_mice_core_ps, Gender == 'female')
CHLD2_HMA_mice_core_female_ps

# Let's see how many core CHLD2 donor ASVs were found in at least one CHLD2 HMA female mouse fecal sample 
prevdf_CHLD2_core_female_mice <- makePrevDF(CHLD2_HMA_mice_core_female_ps, 1)
prevdf_CHLD2_core_female_mice_table <- as.data.frame(prevdf_CHLD2_core_female_mice[1])
colonized_prevdf_CHLD2_core_female_mice <- prevdf_CHLD2_core_female_mice_table[prevdf_CHLD2_core_female_mice_table$Prevalence >= 1,]
str(colonized_prevdf_CHLD2_core_female_mice) # 35 of the 76 core CHLD2 donor ASVs found in at least one CHLD2 mouse fecal sample. These are the 'COLONIZERS'
core_CHLD2_taxa_colonizing_female_mice <-prevdf_CHLD2_core_female_mice[2]
core_CHLD2_taxa_colonizing_female_mice <- unlist(core_CHLD2_taxa_colonizing_female_mice) 
core_CHLD2_taxa_colonizing_female_mice

# split by description to get weekly .....
# Prevalence of ASVs at 48 HRS
CHLD2_HMA_female_mice_core_ps_48HRS <- subset_samples(CHLD2_HMA_mice_core_female_ps, Description == '48hrs_post_inoc')
CHLD2_HMA_female_mice_core_ps_48HRS

prevdf_48HRS <- makePrevDF(CHLD2_HMA_female_mice_core_ps_48HRS,3) 
prevdf_48HRS_table <- as.data.frame(prevdf_48HRS[1])
View(prevdf_48HRS_table)
colonized_prevdf_48HRS <- prevdf_48HRS_table[prevdf_48HRS_table$Prevalence >= 3,]
colonized_prevdf_48HRS$ASV_ID <- rownames(colonized_prevdf_48HRS)
...
# Prevalence of ASVs at 1 week
CHLD2_HMA_female_mice_core_ps_1week <- subset_samples(CHLD2_HMA_mice_core_female_ps, Description == '1week_post_inoc')
CHLD2_HMA_female_mice_core_ps_1week

prevdf_1week <- makePrevDF(CHLD2_HMA_female_mice_core_ps_1week,3)
prevdf_1week_table <- as.data.frame(prevdf_1week[1])
colonized_prevdf_1week <- prevdf_1week_table[prevdf_1week_table$Prevalence >= 3,]
colonized_prevdf_1week$ASV_ID <- rownames(colonized_prevdf_1week)
colonized_prevdf_1week <- colonized_prevdf_1week[,c(3,1,2)]
...
# Prevalence of ASVs at 2 weeks
CHLD2_HMA_female_mice_core_ps_2weeks <- subset_samples(CHLD2_HMA_mice_core_female_ps, Description == '2weeks_post_inoc')
CHLD2_HMA_female_mice_core_ps_2weeks

prevdf_2weeks <- makePrevDF(CHLD2_HMA_female_mice_core_ps_2weeks,3)
prevdf_2weeks_table <- as.data.frame(prevdf_2weeks[1])
View(prevdf_2weeks_table)
colonized_prevdf_2weeks <- prevdf_2weeks_table[prevdf_2weeks_table$Prevalence >= 3,]
colonized_prevdf_2weeks$ASV_ID <- rownames(colonized_prevdf_2weeks)
...
# Prevalence of ASVs at 3 weeks
CHLD2_HMA_female_mice_core_ps_3weeks <- subset_samples(CHLD2_HMA_mice_core_female_ps, Description == '3weeks_post_inoc')
CHLD2_HMA_female_mice_core_ps_3weeks

prevdf_3weeks <- makePrevDF(CHLD2_HMA_female_mice_core_ps_3weeks,3)
prevdf_3weeks_table <- as.data.frame(prevdf_3weeks[1])
colonized_prevdf_3weeks <- prevdf_3weeks_table[prevdf_3weeks_table$Prevalence >= 3,]
colonized_prevdf_3weeks$ASV_ID <- rownames(colonized_prevdf_3weeks)
colonized_prevdf_3weeks <- colonized_prevdf_3weeks[,c(3,1,2)]
...
# Prevalence of ASVs at 4 weeks
CHLD2_HMA_female_mice_core_ps_4weeks <- subset_samples(CHLD2_HMA_mice_core_female_ps, Description == '4weeks_post_inoc')
CHLD2_HMA_female_mice_core_ps_4weeks

prevdf_4weeks <- makePrevDF(CHLD2_HMA_female_mice_core_ps_4weeks,3)
prevdf_4weeks_table <- as.data.frame(prevdf_4weeks[1])
View(prevdf_4weeks_table)
colonized_prevdf_4weeks <- prevdf_4weeks_table[prevdf_4weeks_table$Prevalence >= 3,]
colonized_prevdf_4weeks$ASV_ID <- rownames(colonized_prevdf_4weeks)
...
# Prevalence of ASVs at 5 weeks
CHLD2_HMA_female_mice_core_ps_5weeks <- subset_samples(CHLD2_HMA_mice_core_female_ps, Description == '5weeks_post_inoc')
CHLD2_HMA_female_mice_core_ps_5weeks

prevdf_5weeks <- makePrevDF(CHLD2_HMA_female_mice_core_ps_5weeks,3)
prevdf_5weeks_table <- as.data.frame(prevdf_5weeks[1])
colonized_prevdf_5weeks <- prevdf_5weeks_table[prevdf_5weeks_table$Prevalence >= 3,]
colonized_prevdf_5weeks$ASV_ID <- rownames(colonized_prevdf_5weeks)
colonized_prevdf_5weeks <- colonized_prevdf_5weeks[,c(3,1,2)]
...
# Prevalence of ASVs at 40 days
CHLD2_HMA_female_mice_core_ps_40days <- subset_samples(CHLD2_HMA_mice_core_female_ps, Description == 'Final_collection')
CHLD2_HMA_female_mice_core_ps_40days

prevdf_40days <- makePrevDF(CHLD2_HMA_female_mice_core_ps_40days,3)
prevdf_40days_table <- as.data.frame(prevdf_40days[1])
colonized_prevdf_40days <- prevdf_40days_table[prevdf_40days_table$Prevalence >= 3,]
colonized_prevdf_40days$ASV_ID <- rownames(colonized_prevdf_40days)
colonized_prevdf_40days <- colonized_prevdf_40days[,c(3,1,2)]
...
# Merging all dataframes together
merged_prev_df_CHLD2_core_female_mice <- Reduce(function(x, y) merge(x, y, all=TRUE), list(colonized_prevdf_48HRS_relv, colonized_prevdf_1week_relv, colonized_prevdf_2weeks_relv, colonized_prevdf_3weeks_relv, colonized_prevdf_4weeks_relv, colonized_prevdf_5weeks_relv, colonized_prevdf_40days_relv))
str(merged_prev_df_CHLD2_core_female_mice) # All columns considered factors
merged_prev_df_CHLD2_core_female_mice[2:8] <- lapply(merged_prev_df_CHLD2_core_female_mice[2:8], as.numeric) # need to convert columns 2-8 to numeric in order to do 'rowSums'
str(merged_prev_df_CHLD2_core_female_mice)
View(merged_prev_df_CHLD2_core_female_mice)
     
merged_prev_df_CHLD2_core_female_mice[is.na(merged_prev_df_CHLD2_core_female_mice)] <- 0 # Have to replace 'NA' (missing data) with '0' in order for 'rowSums' to work properly
merged_prev_df_CHLD2_core_female_mice$total <- rowSums(merged_prev_df_CHLD2_core_female_mice[2:8]) # add the values in the rows and use them to populate a new column called 'total'
View(merged_prev_df_CHLD2_core_female_mice)
merged_prev_df_CHLD2_core_female_mice$ASV_ID <- as.character(as.factor(merged_prev_df_CHLD2_core_female_mice$ASV_ID)) # convert 'ASV_ID' column to a character vector
str(merged_prev_df_CHLD2_core_female_mice)
write.table(merged_prev_df_CHLD2_core_female_mice, "merged_prev_df_CHLD2_core_female_mice.txt", sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)

# temporally colonizing taxa defined as taxa found in 2/3 of animals in at least 1 time point (these are all the ASVs included under the 'ASV_ID' column)
temporally_colonizing_core_taxa_CHLD2_female_mice <- merged_prev_df_CHLD2_core_female_mice[,1]
temporally_colonizing_core_taxa_CHLD2_female_mice # 33 core ASVs in female mice

# actually colonizing taxa defined as taxa found in 2/3 of animals in at least 3 time points
actually_colonizing_core_taxa_CHLD2_female_mice_df <- subset(merged_prev_df_CHLD2_core_female_mice, total >= '3') # select the ASVs which were detected in at least 3 time points
View(actually_colonizing_core_taxa_CHLD2_female_mice_df)
actually_colonizing_core_taxa_CHLD2_female_mice <- actually_colonizing_core_taxa_CHLD2_female_mice_df[,1]
actually_colonizing_core_taxa_CHLD2_female_mice # 27 ASVs colonizing mice

# persistent colonizers defined as taxa detected in all >= 4 time points
perst_colonizing_core_CHLD2_female_mice_df <- subset(merged_prev_df_CHLD2_core_female_mice, total >= '4') # select the ASVs which were detected in at least 4 time points
View(perst_colonizing_core_CHLD2_female_mice_df)
perst_colonizing_core_CHLD2_female_mice <- perst_colonizing_core_CHLD2_female_mice_df[,1]
str(perst_colonizing_core_CHLD2_female_mice) # 25 ASVs persistently colonizing mice. These are the 'PERSISTENT COLONIZERS'.

# true colonizers defined as taxa detected in all >= 5 time points
truly_colonizing_core_taxa_CHLD2_female_mice_df <- subset(merged_prev_df_CHLD2_core_female_mice, total >= '5') # select the ASVs which were detected in at least 5 time points
View(truly_colonizing_core_taxa_CHLD2_female_mice_df)
truly_colonizing_core_taxa_CHLD2_female_mice <- truly_colonizing_core_taxa_CHLD2_female_mice_df[,1]
truly_colonizing_core_taxa_CHLD2_female_mice # 23 ASVs 'truly' colonizing mice


core_CHLD2_persisters_in_both_genders_of_mice <- Reduce(intersect, list(perst_colonizing_core_CHLD2_female_mice ,perst_colonizing_core_CHLD2_male_mice))
core_CHLD2_persisters_in_both_genders_of_mice
...
## Male mice
ADLT2_HMA_mice_core_male_ps <- subset_samples(ADLT2_HMA_mice_core_ps, Gender == 'male')
ADLT2_HMA_mice_core_male_ps

# Let's see how many core ADLT2 donor ASVs were found in at least one ADLT2 HMA male mouse fecal sample 
prevdf_ADLT2_core_male_mice <- makePrevDF(ADLT2_HMA_mice_core_male_ps, 1)
prevdf_ADLT2_core_male_mice_table <- as.data.frame(prevdf_ADLT2_core_male_mice[1])
colonized_prevdf_ADLT2_core_male_mice <- prevdf_ADLT2_core_male_mice_table[prevdf_ADLT2_core_male_mice_table$Prevalence >= 1,]
View(colonized_prevdf_ADLT2_core_male_mice) # 47 of the 140 core ADLT2 donor ASVs found in at least one ADLT2 mouse fecal sample. These are the 'COLONIZERS'
core_ADLT2_taxa_colonizing_male_mice <-prevdf_ADLT2_core_male_mice[2]
core_ADLT2_taxa_colonizing_male_mice <- unlist(core_ADLT2_taxa_colonizing_male_mice) 
core_ADLT2_taxa_colonizing_male_mice


# split by description to get weekly .....
# Prevalence of ASVs at 48 HRS
ADLT2_HMA_male_mice_core_ps_48HRS <- subset_samples(ADLT2_HMA_mice_core_male_ps, Description == '48hrs_post_inoc')
ADLT2_HMA_male_mice_core_ps_48HRS

prevdf_48HRS <- makePrevDF(ADLT2_HMA_male_mice_core_ps_48HRS,3) 
prevdf_48HRS_table <- as.data.frame(prevdf_48HRS[1])
colonized_prevdf_48HRS <- prevdf_48HRS_table[prevdf_48HRS_table$Prevalence >= 3,]
View(colonized_prevdf_48HRS)
colonized_prevdf_48HRS$ASV_ID <- rownames(colonized_prevdf_48HRS)
...
# Prevalence of ASVs at 1 week
ADLT2_HMA_male_mice_core_ps_1week <- subset_samples(ADLT2_HMA_mice_core_male_ps, Description == '1week_post_inoc')
ADLT2_HMA_male_mice_core_ps_1week

prevdf_1week <- makePrevDF(ADLT2_HMA_male_mice_core_ps_1week,3)
prevdf_1week_table <- as.data.frame(prevdf_1week[1])
colonized_prevdf_1week <- prevdf_1week_table[prevdf_1week_table$Prevalence >= 3,]
colonized_prevdf_1week$ASV_ID <- rownames(colonized_prevdf_1week)
colonized_prevdf_1week <- colonized_prevdf_1week[,c(3,1,2)]
...
# Prevalence of ASVs at 2 weeks
ADLT2_HMA_male_mice_core_ps_2weeks <- subset_samples(ADLT2_HMA_mice_core_male_ps, Description == '2weeks_post_inoc')
ADLT2_HMA_male_mice_core_ps_2weeks

prevdf_2weeks <- makePrevDF(ADLT2_HMA_male_mice_core_ps_2weeks,3)
prevdf_2weeks_table <- as.data.frame(prevdf_2weeks[1])
colonized_prevdf_2weeks <- prevdf_2weeks_table[prevdf_2weeks_table$Prevalence >= 3,]
colonized_prevdf_2weeks$ASV_ID <- rownames(colonized_prevdf_2weeks)
colonized_prevdf_2weeks <- colonized_prevdf_2weeks[,c(3,1,2)]
...
# Prevalence of ASVs at 3 weeks
ADLT2_HMA_male_mice_core_ps_3weeks <- subset_samples(ADLT2_HMA_mice_core_male_ps, Description == '3weeks_post_inoc')
ADLT2_HMA_male_mice_core_ps_3weeks

prevdf_3weeks <- makePrevDF(ADLT2_HMA_male_mice_core_ps_3weeks,3)
prevdf_3weeks_table <- as.data.frame(prevdf_3weeks[1])
colonized_prevdf_3weeks <- prevdf_3weeks_table[prevdf_3weeks_table$Prevalence >= 3,]
colonized_prevdf_3weeks$ASV_ID <- rownames(colonized_prevdf_3weeks)
colonized_prevdf_3weeks <- colonized_prevdf_3weeks[,c(3,1,2)]
...
# Prevalence of ASVs at 4 weeks
ADLT2_HMA_male_mice_core_ps_4weeks <- subset_samples(ADLT2_HMA_mice_core_male_ps, Description == '4weeks_post_inoc')
ADLT2_HMA_male_mice_core_ps_4weeks

prevdf_4weeks <- makePrevDF(ADLT2_HMA_male_mice_core_ps_4weeks,3)
prevdf_4weeks_table <- as.data.frame(prevdf_4weeks[1])
colonized_prevdf_4weeks <- prevdf_4weeks_table[prevdf_4weeks_table$Prevalence >= 3,]
colonized_prevdf_4weeks$ASV_ID <- rownames(colonized_prevdf_4weeks)
colonized_prevdf_4weeks <- colonized_prevdf_4weeks[,c(3,1,2)]
...
# Prevalence of ASVs at 5 weeks
ADLT2_HMA_male_mice_core_ps_5weeks <- subset_samples(ADLT2_HMA_mice_core_male_ps, Description == '5weeks_post_inoc')
ADLT2_HMA_male_mice_core_ps_5weeks

prevdf_5weeks <- makePrevDF(ADLT2_HMA_male_mice_core_ps_5weeks,3)
prevdf_5weeks_table <- as.data.frame(prevdf_5weeks[1])
colonized_prevdf_5weeks <- prevdf_5weeks_table[prevdf_5weeks_table$Prevalence >= 3,]
colonized_prevdf_5weeks$ASV_ID <- rownames(colonized_prevdf_5weeks)
colonized_prevdf_5weeks <- colonized_prevdf_5weeks[,c(3,1,2)]
...
# Prevalence of ASVs at 40 days
ADLT2_HMA_male_mice_core_ps_40days <- subset_samples(ADLT2_HMA_mice_core_male_ps, Description == 'Final_collection')
ADLT2_HMA_male_mice_core_ps_40days

prevdf_40days <- makePrevDF(ADLT2_HMA_male_mice_core_ps_40days,3)
prevdf_40days_table <- as.data.frame(prevdf_40days[1])
colonized_prevdf_40days <- prevdf_40days_table[prevdf_40days_table$Prevalence >= 3,]
colonized_prevdf_40days$ASV_ID <- rownames(colonized_prevdf_40days)
colonized_prevdf_40days <- colonized_prevdf_40days[,c(3,1,2)]
...
# Merging all dataframes together
merged_prev_df_ADLT2_core_male_mice <- Reduce(function(x, y) merge(x, y, all=TRUE), list(colonized_prevdf_48HRS_relv, colonized_prevdf_1week_relv, colonized_prevdf_2weeks_relv, colonized_prevdf_3weeks_relv, colonized_prevdf_4weeks_relv, colonized_prevdf_5weeks_relv, colonized_prevdf_40days_relv))
str(merged_prev_df_ADLT2_core_male_mice) # All columns considered factors
merged_prev_df_ADLT2_core_male_mice[2:8] <- lapply(merged_prev_df_ADLT2_core_male_mice[2:8], as.numeric) # need to convert columns 2-8 to numeric in order to do 'rowSums'
str(merged_prev_df_ADLT2_core_male_mice)
View(merged_prev_df_ADLT2_core_male_mice)
     
merged_prev_df_ADLT2_core_male_mice[is.na(merged_prev_df_ADLT2_core_male_mice)] <- 0 # Have to replace 'NA' (missing data) with '0' in order for 'rowSums' to work properly
merged_prev_df_ADLT2_core_male_mice$total <- rowSums(merged_prev_df_ADLT2_core_male_mice[2:8]) # add the values in the rows and use them to populate a new column called 'total'
View(merged_prev_df_ADLT2_core_male_mice)
merged_prev_df_ADLT2_core_male_mice$ASV_ID <- as.character(as.factor(merged_prev_df_ADLT2_core_male_mice$ASV_ID)) # convert 'ASV_ID' column to a character vector
str(merged_prev_df_ADLT2_core_male_mice)
write.table(merged_prev_df_ADLT2_core_male_mice, "merged_prev_df_ADLT2_core_male_mice.txt", sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)

# temporally colonizing taxa defined as taxa found in 2/3 of animals in at least 1 time point (these are all the ASVs included under the 'ASV_ID' column)
temporally_colonizing_core_taxa_ADLT2_male_mice <- merged_prev_df_ADLT2_core_male_mice[,1]
temporally_colonizing_core_taxa_ADLT2_male_mice # 40 core ASVs in male mice

# actually colonizing taxa defined as taxa found in 2/3 of animals in at least 3 time points
actually_colonizing_core_taxa_ADLT2_male_mice_df <- subset(merged_prev_df_ADLT2_core_male_mice, total >= '3') # select the ASVs which were detected in at least 3 time points
View(actually_colonizing_core_taxa_ADLT2_male_mice_df)
actually_colonizing_core_taxa_ADLT2_male_mice <- actually_colonizing_core_taxa_ADLT2_male_mice_df[,1]
actually_colonizing_core_taxa_ADLT2_male_mice # 33 ASVs colonizing male mice

# persistent colonizers defined as taxa detected in all >= 4 time points
perst_colonizing_core_ADLT2_male_mice_df <- subset(merged_prev_df_ADLT2_core_male_mice, total >= '4') # select the ASVs which were detected in at least 4 time points
View(perst_colonizing_core_ADLT2_male_mice_df)
perst_colonizing_core_ADLT2_male_mice <- perst_colonizing_core_ADLT2_male_mice_df[,1]
perst_colonizing_core_ADLT2_male_mice # 32 ASVs persistently colonizing male mice. These are the 'PERSISTENT COLONIZERS'.

# true colonizers defined as taxa detected in all >= 5 time points
truly_colonizing_core_taxa_ADLT2_male_mice_df <- subset(merged_prev_df_ADLT2_core_male_mice, total >= '5') # select the ASVs which were detected in at least 5 time points
View(truly_colonizing_core_taxa_ADLT2_male_mice_df)
truly_colonizing_core_taxa_ADLT2_male_mice <- truly_colonizing_core_taxa_ADLT2_male_mice_df[,1]
truly_colonizing_core_taxa_ADLT2_male_mice # 30 ASVs 'truly' colonizing male mice


## Female mice
ADLT2_HMA_mice_core_female_ps <- subset_samples(ADLT2_HMA_mice_core_ps, Gender == 'female')
ADLT2_HMA_mice_core_female_ps

# Let's see how many core ADLT2 donor ASVs were found in at least one ADLT2 HMA female mouse fecal sample 
prevdf_ADLT2_core_female_mice <- makePrevDF(ADLT2_HMA_mice_core_female_ps, 1)
prevdf_ADLT2_core_female_mice_table <- as.data.frame(prevdf_ADLT2_core_female_mice[1])
colonized_prevdf_ADLT2_core_female_mice <- prevdf_ADLT2_core_female_mice_table[prevdf_ADLT2_core_female_mice_table$Prevalence >= 1,]
View(colonized_prevdf_ADLT2_core_female_mice) # 63 of the 140 core ADLT2 donor ASVs found in at least one ADLT2 mouse fecal sample. These are the 'COLONIZERS'
core_ADLT2_taxa_colonizing_female_mice <-prevdf_ADLT2_core_female_mice[2]
core_ADLT2_taxa_colonizing_female_mice <- unlist(core_ADLT2_taxa_colonizing_female_mice) 
core_ADLT2_taxa_colonizing_female_mice

# split by description to get weekly .....
# Prevalence of ASVs at 48 HRS
ADLT2_HMA_female_mice_core_ps_48HRS <- subset_samples(ADLT2_HMA_mice_core_female_ps, Description == '48hrs_post_inoc')
ADLT2_HMA_female_mice_core_ps_48HRS

prevdf_48HRS <- makePrevDF(ADLT2_HMA_female_mice_core_ps_48HRS,3) 
prevdf_48HRS_table <- as.data.frame(prevdf_48HRS[1])
View(prevdf_48HRS_table)
colonized_prevdf_48HRS <- prevdf_48HRS_table[prevdf_48HRS_table$Prevalence >= 3,]
colonized_prevdf_48HRS$ASV_ID <- rownames(colonized_prevdf_48HRS)
...
# Prevalence of ASVs at 1 week
ADLT2_HMA_female_mice_core_ps_1week <- subset_samples(ADLT2_HMA_mice_core_female_ps, Description == '1week_post_inoc')
ADLT2_HMA_female_mice_core_ps_1week

prevdf_1week <- makePrevDF(ADLT2_HMA_female_mice_core_ps_1week,3)
prevdf_1week_table <- as.data.frame(prevdf_1week[1])
colonized_prevdf_1week <- prevdf_1week_table[prevdf_1week_table$Prevalence >= 3,]
colonized_prevdf_1week$ASV_ID <- rownames(colonized_prevdf_1week)
colonized_prevdf_1week <- colonized_prevdf_1week[,c(3,1,2)]
...
# Prevalence of ASVs at 2 weeks
ADLT2_HMA_female_mice_core_ps_2weeks <- subset_samples(ADLT2_HMA_mice_core_female_ps, Description == '2weeks_post_inoc')
ADLT2_HMA_female_mice_core_ps_2weeks

prevdf_2weeks <- makePrevDF(ADLT2_HMA_female_mice_core_ps_2weeks,3)
prevdf_2weeks_table <- as.data.frame(prevdf_2weeks[1])
colonized_prevdf_2weeks <- prevdf_2weeks_table[prevdf_2weeks_table$Prevalence >= 3,]
colonized_prevdf_2weeks$ASV_ID <- rownames(colonized_prevdf_2weeks)
colonized_prevdf_2weeks <- colonized_prevdf_2weeks[,c(3,1,2)]
...
# Prevalence of ASVs at 3 weeks
ADLT2_HMA_female_mice_core_ps_3weeks <- subset_samples(ADLT2_HMA_mice_core_female_ps, Description == '3weeks_post_inoc')
ADLT2_HMA_female_mice_core_ps_3weeks

prevdf_3weeks <- makePrevDF(ADLT2_HMA_female_mice_core_ps_3weeks,3)
prevdf_3weeks_table <- as.data.frame(prevdf_3weeks[1])
colonized_prevdf_3weeks <- prevdf_3weeks_table[prevdf_3weeks_table$Prevalence >= 3,]
colonized_prevdf_3weeks$ASV_ID <- rownames(colonized_prevdf_3weeks)
colonized_prevdf_3weeks <- colonized_prevdf_3weeks[,c(3,1,2)]
...
# Prevalence of ASVs at 4 weeks
ADLT2_HMA_female_mice_core_ps_4weeks <- subset_samples(ADLT2_HMA_mice_core_female_ps, Description == '4weeks_post_inoc')
ADLT2_HMA_female_mice_core_ps_4weeks

prevdf_4weeks <- makePrevDF(ADLT2_HMA_female_mice_core_ps_4weeks,3)
prevdf_4weeks_table <- as.data.frame(prevdf_4weeks[1])
colonized_prevdf_4weeks <- prevdf_4weeks_table[prevdf_4weeks_table$Prevalence >= 3,]
colonized_prevdf_4weeks$ASV_ID <- rownames(colonized_prevdf_4weeks)
colonized_prevdf_4weeks <- colonized_prevdf_4weeks[,c(3,1,2)]
...
# Prevalence of ASVs at 5 weeks
ADLT2_HMA_female_mice_core_ps_5weeks <- subset_samples(ADLT2_HMA_mice_core_female_ps, Description == '5weeks_post_inoc')
ADLT2_HMA_female_mice_core_ps_5weeks

prevdf_5weeks <- makePrevDF(ADLT2_HMA_female_mice_core_ps_5weeks,3)
prevdf_5weeks_table <- as.data.frame(prevdf_5weeks[1])
colonized_prevdf_5weeks <- prevdf_5weeks_table[prevdf_5weeks_table$Prevalence >= 3,]
colonized_prevdf_5weeks$ASV_ID <- rownames(colonized_prevdf_5weeks)
colonized_prevdf_5weeks <- colonized_prevdf_5weeks[,c(3,1,2)]
...
# Prevalence of ASVs at 40 days
ADLT2_HMA_female_mice_core_ps_40days <- subset_samples(ADLT2_HMA_mice_core_female_ps, Description == 'Final_collection')
ADLT2_HMA_female_mice_core_ps_40days

prevdf_40days <- makePrevDF(ADLT2_HMA_female_mice_core_ps_40days,3)
prevdf_40days_table <- as.data.frame(prevdf_40days[1])
colonized_prevdf_40days <- prevdf_40days_table[prevdf_40days_table$Prevalence >= 3,]
colonized_prevdf_40days$ASV_ID <- rownames(colonized_prevdf_40days)
colonized_prevdf_40days <- colonized_prevdf_40days[,c(3,1,2)]
...
# Merging all dataframes together
merged_prev_df_ADLT2_core_female_mice <- Reduce(function(x, y) merge(x, y, all=TRUE), list(colonized_prevdf_48HRS_relv, colonized_prevdf_1week_relv, colonized_prevdf_2weeks_relv, colonized_prevdf_3weeks_relv, colonized_prevdf_4weeks_relv, colonized_prevdf_5weeks_relv, colonized_prevdf_40days_relv))
str(merged_prev_df_ADLT2_core_female_mice) # All columns considered factors
merged_prev_df_ADLT2_core_female_mice[2:8] <- lapply(merged_prev_df_ADLT2_core_female_mice[2:8], as.numeric) # need to convert columns 2-8 to numeric in order to do 'rowSums'
str(merged_prev_df_ADLT2_core_female_mice)
View(merged_prev_df_ADLT2_core_female_mice)
     
merged_prev_df_ADLT2_core_female_mice[is.na(merged_prev_df_ADLT2_core_female_mice)] <- 0 # Have to replace 'NA' (missing data) with '0' in order for 'rowSums' to work properly
merged_prev_df_ADLT2_core_female_mice$total <- rowSums(merged_prev_df_ADLT2_core_female_mice[2:8]) # add the values in the rows and use them to populate a new column called 'total'
View(merged_prev_df_ADLT2_core_female_mice)
merged_prev_df_ADLT2_core_female_mice$ASV_ID <- as.character(as.factor(merged_prev_df_ADLT2_core_female_mice$ASV_ID)) # convert 'ASV_ID' column to a character vector
str(merged_prev_df_ADLT2_core_female_mice)
write.table(merged_prev_df_ADLT2_core_female_mice, "merged_prev_df_ADLT2_core_female_mice.txt", sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)

# temporally colonizing taxa defined as taxa found in 2/3 of animals in at least 1 time point (these are all the ASVs included under the 'ASV_ID' column)
temporally_colonizing_core_taxa_ADLT2_female_mice <- merged_prev_df_ADLT2_core_female_mice[,1]
temporally_colonizing_core_taxa_ADLT2_female_mice # 42 core ASVs in female mice

# actually colonizing taxa defined as taxa found in 2/3 of animals in at least 3 time points
actually_colonizing_core_taxa_ADLT2_female_mice_df <- subset(merged_prev_df_ADLT2_core_female_mice, total >= '3') # select the ASVs which were detected in at least 3 time points
View(actually_colonizing_core_taxa_ADLT2_female_mice_df)
actually_colonizing_core_taxa_ADLT2_female_mice <- actually_colonizing_core_taxa_ADLT2_female_mice_df[,1]
actually_colonizing_core_taxa_ADLT2_female_mice # 34 ASVs colonizing mice

# persistent colonizers defined as taxa detected in all >= 4 time points
perst_colonizing_core_ADLT2_female_mice_df <- subset(merged_prev_df_ADLT2_core_female_mice, total >= '4') # select the ASVs which were detected in at least 4 time points
View(perst_colonizing_core_ADLT2_female_mice_df)
perst_colonizing_core_ADLT2_female_mice <- perst_colonizing_core_ADLT2_female_mice_df[,1]
str(perst_colonizing_core_ADLT2_female_mice) # 33 ASVs persistently colonizing mice. These are the 'PERSISTENT COLONIZERS'.

# true colonizers defined as taxa detected in all >= 5 time points
truly_colonizing_core_taxa_ADLT2_female_mice_df <- subset(merged_prev_df_ADLT2_core_female_mice, total >= '5') # select the ASVs which were detected in at least 5 time points
View(truly_colonizing_core_taxa_ADLT2_female_mice_df)
truly_colonizing_core_taxa_ADLT2_female_mice <- truly_colonizing_core_taxa_ADLT2_female_mice_df[,1]
truly_colonizing_core_taxa_ADLT2_female_mice # 30 ASVs 'truly' colonizing mice


core_ADLT2_persisters_in_both_genders_of_mice <- Reduce(intersect, list(perst_colonizing_core_ADLT2_female_mice ,perst_colonizing_core_ADLT2_male_mice))
core_ADLT2_persisters_in_both_genders_of_mice
...
## Male mice
ELD2_HMA_mice_core_male_ps <- subset_samples(ELD2_HMA_mice_core_ps, Gender == 'male')
ELD2_HMA_mice_core_male_ps

# Let's see how many core ELD2 donor ASVs were found in at least one ELD2 HMA male mouse fecal sample 
prevdf_ELD2_core_male_mice <- makePrevDF(ELD2_HMA_mice_core_male_ps, 1)
prevdf_ELD2_core_male_mice_table <- as.data.frame(prevdf_ELD2_core_male_mice[1])
colonized_prevdf_ELD2_core_male_mice <- prevdf_ELD2_core_male_mice_table[prevdf_ELD2_core_male_mice_table$Prevalence >= 1,]
View(colonized_prevdf_ELD2_core_male_mice) # 50 of the 134 core ELD2 donor ASVs found in at least one ELD2 mouse fecal sample. These are the 'COLONIZERS'
core_ELD2_taxa_colonizing_male_mice <-prevdf_ELD2_core_male_mice[2]
core_ELD2_taxa_colonizing_male_mice <- unlist(core_ELD2_taxa_colonizing_male_mice) 
core_ELD2_taxa_colonizing_male_mice

# split by description to get weekly .....
# Prevalence of ASVs at 48 HRS
ELD2_HMA_male_mice_core_ps_48HRS <- subset_samples(ELD2_HMA_mice_core_male_ps, Description == '48hrs_post_inoc')
ELD2_HMA_male_mice_core_ps_48HRS

prevdf_48HRS <- makePrevDF(ELD2_HMA_male_mice_core_ps_48HRS,3) 
prevdf_48HRS_table <- as.data.frame(prevdf_48HRS[1])
colonized_prevdf_48HRS <- prevdf_48HRS_table[prevdf_48HRS_table$Prevalence >= 3,]
View(colonized_prevdf_48HRS)
colonized_prevdf_48HRS$ASV_ID <- rownames(colonized_prevdf_48HRS)
...
# Prevalence of ASVs at 1 week
ELD2_HMA_male_mice_core_ps_1week <- subset_samples(ELD2_HMA_mice_core_male_ps, Description == '1week_post_inoc')
ELD2_HMA_male_mice_core_ps_1week

prevdf_1week <- makePrevDF(ELD2_HMA_male_mice_core_ps_1week,3)
prevdf_1week_table <- as.data.frame(prevdf_1week[1])
colonized_prevdf_1week <- prevdf_1week_table[prevdf_1week_table$Prevalence >= 3,]
colonized_prevdf_1week$ASV_ID <- rownames(colonized_prevdf_1week)
colonized_prevdf_1week <- colonized_prevdf_1week[,c(3,1,2)]
...
# Prevalence of ASVs at 2 weeks
ELD2_HMA_male_mice_core_ps_2weeks <- subset_samples(ELD2_HMA_mice_core_male_ps, Description == '2weeks_post_inoc')
ELD2_HMA_male_mice_core_ps_2weeks

prevdf_2weeks <- makePrevDF(ELD2_HMA_male_mice_core_ps_2weeks,3)
prevdf_2weeks_table <- as.data.frame(prevdf_2weeks[1])
colonized_prevdf_2weeks <- prevdf_2weeks_table[prevdf_2weeks_table$Prevalence >= 3,]
colonized_prevdf_2weeks$ASV_ID <- rownames(colonized_prevdf_2weeks)
colonized_prevdf_2weeks <- colonized_prevdf_2weeks[,c(3,1,2)]
...
# Prevalence of ASVs at 3 weeks
ELD2_HMA_male_mice_core_ps_3weeks <- subset_samples(ELD2_HMA_mice_core_male_ps, Description == '3weeks_post_inoc')
ELD2_HMA_male_mice_core_ps_3weeks

prevdf_3weeks <- makePrevDF(ELD2_HMA_male_mice_core_ps_3weeks,3)
prevdf_3weeks_table <- as.data.frame(prevdf_3weeks[1])
colonized_prevdf_3weeks <- prevdf_3weeks_table[prevdf_3weeks_table$Prevalence >= 3,]
colonized_prevdf_3weeks$ASV_ID <- rownames(colonized_prevdf_3weeks)
colonized_prevdf_3weeks <- colonized_prevdf_3weeks[,c(3,1,2)]
...
# Prevalence of ASVs at 4 weeks
ELD2_HMA_male_mice_core_ps_4weeks <- subset_samples(ELD2_HMA_mice_core_male_ps, Description == '4weeks_post_inoc')
ELD2_HMA_male_mice_core_ps_4weeks

prevdf_4weeks <- makePrevDF(ELD2_HMA_male_mice_core_ps_4weeks,3)
prevdf_4weeks_table <- as.data.frame(prevdf_4weeks[1])
colonized_prevdf_4weeks <- prevdf_4weeks_table[prevdf_4weeks_table$Prevalence >= 3,]
colonized_prevdf_4weeks$ASV_ID <- rownames(colonized_prevdf_4weeks)
colonized_prevdf_4weeks <- colonized_prevdf_4weeks[,c(3,1,2)]
...
# Prevalence of ASVs at 5 weeks
ELD2_HMA_male_mice_core_ps_5weeks <- subset_samples(ELD2_HMA_mice_core_male_ps, Description == '5weeks_post_inoc')
ELD2_HMA_male_mice_core_ps_5weeks

prevdf_5weeks <- makePrevDF(ELD2_HMA_male_mice_core_ps_5weeks,3)
prevdf_5weeks_table <- as.data.frame(prevdf_5weeks[1])
colonized_prevdf_5weeks <- prevdf_5weeks_table[prevdf_5weeks_table$Prevalence >= 3,]
colonized_prevdf_5weeks$ASV_ID <- rownames(colonized_prevdf_5weeks)
colonized_prevdf_5weeks <- colonized_prevdf_5weeks[,c(3,1,2)]
...
# Prevalence of ASVs at 40 days
ELD2_HMA_male_mice_core_ps_40days <- subset_samples(ELD2_HMA_mice_core_male_ps, Description == 'Final_collection')
ELD2_HMA_male_mice_core_ps_40days

prevdf_40days <- makePrevDF(ELD2_HMA_male_mice_core_ps_40days,3)
prevdf_40days_table <- as.data.frame(prevdf_40days[1])
colonized_prevdf_40days <- prevdf_40days_table[prevdf_40days_table$Prevalence >= 3,]
colonized_prevdf_40days$ASV_ID <- rownames(colonized_prevdf_40days)
colonized_prevdf_40days <- colonized_prevdf_40days[,c(3,1,2)]
...
# Merging all dataframes together
merged_prev_df_ELD2_core_male_mice <- Reduce(function(x, y) merge(x, y, all=TRUE), list(colonized_prevdf_48HRS_relv, colonized_prevdf_1week_relv, colonized_prevdf_2weeks_relv, colonized_prevdf_3weeks_relv, colonized_prevdf_4weeks_relv, colonized_prevdf_5weeks_relv, colonized_prevdf_40days_relv))
str(merged_prev_df_ELD2_core_male_mice) # All columns considered factors
merged_prev_df_ELD2_core_male_mice[2:8] <- lapply(merged_prev_df_ELD2_core_male_mice[2:8], as.numeric) # need to convert columns 2-8 to numeric in order to do 'rowSums'
str(merged_prev_df_ELD2_core_male_mice)
View(merged_prev_df_ELD2_core_male_mice)
     
merged_prev_df_ELD2_core_male_mice[is.na(merged_prev_df_ELD2_core_male_mice)] <- 0 # Have to replace 'NA' (missing data) with '0' in order for 'rowSums' to work properly
merged_prev_df_ELD2_core_male_mice$total <- rowSums(merged_prev_df_ELD2_core_male_mice[2:8]) # add the values in the rows and use them to populate a new column called 'total'
View(merged_prev_df_ELD2_core_male_mice)
merged_prev_df_ELD2_core_male_mice$ASV_ID <- as.character(as.factor(merged_prev_df_ELD2_core_male_mice$ASV_ID)) # convert 'ASV_ID' column to a character vector
str(merged_prev_df_ELD2_core_male_mice)
write.table(merged_prev_df_ELD2_core_male_mice, "merged_prev_df_ELD2_core_male_mice.txt", sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)

# temporally colonizing taxa defined as taxa found in 2/3 of animals in at least 1 time point (these are all the ASVs included under the 'ASV_ID' column)
temporally_colonizing_core_taxa_ELD2_male_mice <- merged_prev_df_ELD2_core_male_mice[,1]
temporally_colonizing_core_taxa_ELD2_male_mice # 41 core ASVs in male mice

# actually colonizing taxa defined as taxa found in 2/3 of animals in at least 3 time points
actually_colonizing_core_taxa_ELD2_male_mice_df <- subset(merged_prev_df_ELD2_core_male_mice, total >= '3') # select the ASVs which were detected in at least 3 time points
View(actually_colonizing_core_taxa_ELD2_male_mice_df)
actually_colonizing_core_taxa_ELD2_male_mice <- actually_colonizing_core_taxa_ELD2_male_mice_df[,1]
actually_colonizing_core_taxa_ELD2_male_mice # 35 ASVs colonizing male mice

# persistent colonizers defined as taxa detected in all >= 4 time points
perst_colonizing_core_ELD2_male_mice_df <- subset(merged_prev_df_ELD2_core_male_mice, total >= '4') # select the ASVs which were detected in at least 4 time points
View(perst_colonizing_core_ELD2_male_mice_df)
perst_colonizing_core_ELD2_male_mice <- perst_colonizing_core_ELD2_male_mice_df[,1]
perst_colonizing_core_ELD2_male_mice # 30 ASVs persistently colonizing male mice. These are the 'PERSISTENT COLONIZERS'.

# true colonizers defined as taxa detected in all >= 5 time points
truly_colonizing_core_taxa_ELD2_male_mice_df <- subset(merged_prev_df_ELD2_core_male_mice, total >= '5') # select the ASVs which were detected in at least 5 time points
View(truly_colonizing_core_taxa_ELD2_male_mice_df)
truly_colonizing_core_taxa_ELD2_male_mice <- truly_colonizing_core_taxa_ELD2_male_mice_df[,1]
truly_colonizing_core_taxa_ELD2_male_mice # 24 ASVs 'truly' colonizing male mice


## Female mice
ELD2_HMA_mice_core_female_ps <- subset_samples(ELD2_HMA_mice_core_ps, Gender == 'female')
ELD2_HMA_mice_core_female_ps

# Let's see how many core ELD2 donor ASVs were found in at least one ELD2 HMA female mouse fecal sample 
prevdf_ELD2_core_female_mice <- makePrevDF(ELD2_HMA_mice_core_female_ps, 1)
prevdf_ELD2_core_female_mice_table <- as.data.frame(prevdf_ELD2_core_female_mice[1])
colonized_prevdf_ELD2_core_female_mice <- prevdf_ELD2_core_female_mice_table[prevdf_ELD2_core_female_mice_table$Prevalence >= '1',]
View(colonized_prevdf_ELD2_core_female_mice) # 51 of the 140 core ELD2 donor ASVs found in at least one ELD2 mouse fecal sample. These are the 'COLONIZERS'
core_ELD2_taxa_colonizing_female_mice <-prevdf_ELD2_core_female_mice[2]
core_ELD2_taxa_colonizing_female_mice <- unlist(core_ELD2_taxa_colonizing_female_mice) 
core_ELD2_taxa_colonizing_female_mice

# split by description to get weekly .....
# Prevalence of ASVs at 48 HRS
ELD2_HMA_female_mice_core_ps_48HRS <- subset_samples(ELD2_HMA_mice_core_female_ps, Description == '48hrs_post_inoc')
ELD2_HMA_female_mice_core_ps_48HRS

prevdf_48HRS <- makePrevDF(ELD2_HMA_female_mice_core_ps_48HRS,3) 
prevdf_48HRS_table <- as.data.frame(prevdf_48HRS[1])
View(prevdf_48HRS_table)
colonized_prevdf_48HRS <- prevdf_48HRS_table[prevdf_48HRS_table$Prevalence >= 3,]
colonized_prevdf_48HRS$ASV_ID <- rownames(colonized_prevdf_48HRS)
...
# Prevalence of ASVs at 1 week
ELD2_HMA_female_mice_core_ps_1week <- subset_samples(ELD2_HMA_mice_core_female_ps, Description == '1week_post_inoc')
ELD2_HMA_female_mice_core_ps_1week

prevdf_1week <- makePrevDF(ELD2_HMA_female_mice_core_ps_1week,3)
prevdf_1week_table <- as.data.frame(prevdf_1week[1])
colonized_prevdf_1week <- prevdf_1week_table[prevdf_1week_table$Prevalence >= 3,]
colonized_prevdf_1week$ASV_ID <- rownames(colonized_prevdf_1week)
colonized_prevdf_1week <- colonized_prevdf_1week[,c(3,1,2)]
...
# Prevalence of ASVs at 2 weeks
ELD2_HMA_female_mice_core_ps_2weeks <- subset_samples(ELD2_HMA_mice_core_female_ps, Description == '2weeks_post_inoc')
ELD2_HMA_female_mice_core_ps_2weeks

prevdf_2weeks <- makePrevDF(ELD2_HMA_female_mice_core_ps_2weeks,3)
prevdf_2weeks_table <- as.data.frame(prevdf_2weeks[1])
colonized_prevdf_2weeks <- prevdf_2weeks_table[prevdf_2weeks_table$Prevalence >= 3,]
colonized_prevdf_2weeks$ASV_ID <- rownames(colonized_prevdf_2weeks)
colonized_prevdf_2weeks <- colonized_prevdf_2weeks[,c(3,1,2)]
...
# Prevalence of ASVs at 3 weeks
ELD2_HMA_female_mice_core_ps_3weeks <- subset_samples(ELD2_HMA_mice_core_female_ps, Description == '3weeks_post_inoc')
ELD2_HMA_female_mice_core_ps_3weeks

prevdf_3weeks <- makePrevDF(ELD2_HMA_female_mice_core_ps_3weeks,3)
prevdf_3weeks_table <- as.data.frame(prevdf_3weeks[1])
colonized_prevdf_3weeks <- prevdf_3weeks_table[prevdf_3weeks_table$Prevalence >= 3,]
colonized_prevdf_3weeks$ASV_ID <- rownames(colonized_prevdf_3weeks)
colonized_prevdf_3weeks <- colonized_prevdf_3weeks[,c(3,1,2)]
...
# Prevalence of ASVs at 4 weeks
ELD2_HMA_female_mice_core_ps_4weeks <- subset_samples(ELD2_HMA_mice_core_female_ps, Description == '4weeks_post_inoc')
ELD2_HMA_female_mice_core_ps_4weeks

prevdf_4weeks <- makePrevDF(ELD2_HMA_female_mice_core_ps_4weeks,3)
prevdf_4weeks_table <- as.data.frame(prevdf_4weeks[1])
colonized_prevdf_4weeks <- prevdf_4weeks_table[prevdf_4weeks_table$Prevalence >= 3,]
colonized_prevdf_4weeks$ASV_ID <- rownames(colonized_prevdf_4weeks)
colonized_prevdf_4weeks <- colonized_prevdf_4weeks[,c(3,1,2)]
...
# Prevalence of ASVs at 5 weeks
ELD2_HMA_female_mice_core_ps_5weeks <- subset_samples(ELD2_HMA_mice_core_female_ps, Description == '5weeks_post_inoc')
ELD2_HMA_female_mice_core_ps_5weeks

prevdf_5weeks <- makePrevDF(ELD2_HMA_female_mice_core_ps_5weeks,3)
prevdf_5weeks_table <- as.data.frame(prevdf_5weeks[1])
colonized_prevdf_5weeks <- prevdf_5weeks_table[prevdf_5weeks_table$Prevalence >= 3,]
colonized_prevdf_5weeks$ASV_ID <- rownames(colonized_prevdf_5weeks)
colonized_prevdf_5weeks <- colonized_prevdf_5weeks[,c(3,1,2)]
...
# Prevalence of ASVs at 40 days
ELD2_HMA_female_mice_core_ps_40days <- subset_samples(ELD2_HMA_mice_core_female_ps, Description == 'Final_collection')
ELD2_HMA_female_mice_core_ps_40days

prevdf_40days <- makePrevDF(ELD2_HMA_female_mice_core_ps_40days,3)
prevdf_40days_table <- as.data.frame(prevdf_40days[1])
colonized_prevdf_40days <- prevdf_40days_table[prevdf_40days_table$Prevalence >= 3,]
colonized_prevdf_40days$ASV_ID <- rownames(colonized_prevdf_40days)
colonized_prevdf_40days <- colonized_prevdf_40days[,c(3,1,2)]
...
# Merging all dataframes together
merged_prev_df_ELD2_core_female_mice <- Reduce(function(x, y) merge(x, y, all=TRUE), list(colonized_prevdf_48HRS_relv, colonized_prevdf_1week_relv, colonized_prevdf_2weeks_relv, colonized_prevdf_3weeks_relv, colonized_prevdf_4weeks_relv, colonized_prevdf_5weeks_relv, colonized_prevdf_40days_relv))
str(merged_prev_df_ELD2_core_female_mice) # All columns considered factors
merged_prev_df_ELD2_core_female_mice[2:8] <- lapply(merged_prev_df_ELD2_core_female_mice[2:8], as.numeric) # need to convert columns 2-8 to numeric in order to do 'rowSums'
str(merged_prev_df_ELD2_core_female_mice)
View(merged_prev_df_ELD2_core_female_mice)
     
merged_prev_df_ELD2_core_female_mice[is.na(merged_prev_df_ELD2_core_female_mice)] <- 0 # Have to replace 'NA' (missing data) with '0' in order for 'rowSums' to work properly
merged_prev_df_ELD2_core_female_mice$total <- rowSums(merged_prev_df_ELD2_core_female_mice[2:8]) # add the values in the rows and use them to populate a new column called 'total'
View(merged_prev_df_ELD2_core_female_mice)
merged_prev_df_ELD2_core_female_mice$ASV_ID <- as.character(as.factor(merged_prev_df_ELD2_core_female_mice$ASV_ID)) # convert 'ASV_ID' column to a character vector
str(merged_prev_df_ELD2_core_female_mice)
write.table(merged_prev_df_ELD2_core_female_mice, "merged_prev_df_ELD2_core_female_mice.txt", sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)

# temporally colonizing taxa defined as taxa found in 2/3 of animals in at least 1 time point (these are all the ASVs included under the 'ASV_ID' column)
temporally_colonizing_core_taxa_ELD2_female_mice <- merged_prev_df_ELD2_core_female_mice[,1]
temporally_colonizing_core_taxa_ELD2_female_mice # 44 core ASVs in female mice

# actually colonizing taxa defined as taxa found in 2/3 of animals in at least 3 time points
actually_colonizing_core_taxa_ELD2_female_mice_df <- subset(merged_prev_df_ELD2_core_female_mice, total >= '3') # select the ASVs which were detected in at least 3 time points
View(actually_colonizing_core_taxa_ELD2_female_mice_df)
actually_colonizing_core_taxa_ELD2_female_mice <- actually_colonizing_core_taxa_ELD2_female_mice_df[,1]
actually_colonizing_core_taxa_ELD2_female_mice # 32 ASVs colonizing mice

# persistent colonizers defined as taxa detected in all >= 4 time points
perst_colonizing_core_ELD2_female_mice_df <- subset(merged_prev_df_ELD2_core_female_mice, total >= '4') # select the ASVs which were detected in at least 4 time points
View(perst_colonizing_core_ELD2_female_mice_df)
perst_colonizing_core_ELD2_female_mice <- perst_colonizing_core_ELD2_female_mice_df[,1]
str(perst_colonizing_core_ELD2_female_mice) # 27 ASVs persistently colonizing mice. These are the 'PERSISTENT COLONIZERS'.

# true colonizers defined as taxa detected in all >= 5 time points
truly_colonizing_core_taxa_ELD2_female_mice_df <- subset(merged_prev_df_ELD2_core_female_mice, total >= '5') # select the ASVs which were detected in at least 5 time points
View(truly_colonizing_core_taxa_ELD2_female_mice_df)
truly_colonizing_core_taxa_ELD2_female_mice <- truly_colonizing_core_taxa_ELD2_female_mice_df[,1]
truly_colonizing_core_taxa_ELD2_female_mice # 26 ASVs 'truly' colonizing mice


core_ELD2_persisters_in_both_genders_of_mice <- Reduce(intersect, list(perst_colonizing_core_ELD2_female_mice ,perst_colonizing_core_ELD2_male_mice))
core_ELD2_persisters_in_both_genders_of_mice 
```



Let's perform a similar analysis for the piglets specifying that to be a persistent colonizer, an ASV had to be in 3/3 piglets instead 2/3 piglets:
...
actually_colonizing_core_taxa_INF2_piglets # 18 ASVs 'actually' colonizing piglets

# persistent colonizers defined as taxa detected in all >= 4 time points
perst_colonizing_core_INF2_piglets_df <- subset(merged_prev_df_INF2_core_piglets, total >= '4') # select the ASVs which were detected in at least 4 time points
View(perst_colonizing_core_INF2_piglets_df)
perst_colonizing_core_INF2_piglets <- perst_colonizing_core_INF2_piglets_df[,1]
str(perst_colonizing_core_INF2_piglets) # 10 ASVs persistently colonizing piglets. These are the 'PERSISTENT COLONIZERS'
...
truly_colonizing_core_taxa_INF2_piglets # 8 ASVs 'truly' colonizing piglets
...
prevdf_48HRS <- makePrevDF(CHLD2_HMA_piglets_core_ps_48HRS,3) 
prevdf_48HRS_table <- as.data.frame(prevdf_48HRS[1])
colonized_prevdf_48HRS <- prevdf_48HRS_table[prevdf_48HRS_table$Prevalence == 3,]
colonized_prevdf_48HRS$ASV_ID <- rownames(colonized_prevdf_48HRS)
colonized_prevdf_48HRS <- colonized_prevdf_48HRS[,c(3,1,2)]
...
prevdf_1week <- makePrevDF(CHLD2_HMA_piglets_core_ps_1week,3)
prevdf_1week_table <- as.data.frame(prevdf_1week[1])
colonized_prevdf_1week <- prevdf_1week_table[prevdf_1week_table$Prevalence == 3,]
colonized_prevdf_1week$ASV_ID <- rownames(colonized_prevdf_1week)
colonized_prevdf_1week <- colonized_prevdf_1week[,c(3,1,2)]
...
prevdf_2weeks <- makePrevDF(CHLD2_HMA_piglets_core_ps_2weeks,3)
prevdf_2weeks_table <- as.data.frame(prevdf_2weeks[1])
colonized_prevdf_2weeks <- prevdf_2weeks_table[prevdf_2weeks_table$Prevalence == 3,]
colonized_prevdf_2weeks$ASV_ID <- rownames(colonized_prevdf_2weeks)
colonized_prevdf_2weeks <- colonized_prevdf_2weeks[,c(3,1,2)]
...
prevdf_3weeks <- makePrevDF(CHLD2_HMA_piglets_core_ps_3weeks,3)
prevdf_3weeks_table <- as.data.frame(prevdf_3weeks[1])
colonized_prevdf_3weeks <- prevdf_3weeks_table[prevdf_3weeks_table$Prevalence == 3,]
colonized_prevdf_3weeks$ASV_ID <- rownames(colonized_prevdf_3weeks)
colonized_prevdf_3weeks <- colonized_prevdf_3weeks[,c(3,1,2)]
...
prevdf_4weeks <- makePrevDF(CHLD2_HMA_piglets_core_ps_4weeks,3)
prevdf_4weeks_table <- as.data.frame(prevdf_4weeks[1])
colonized_prevdf_4weeks <- prevdf_4weeks_table[prevdf_4weeks_table$Prevalence == 3,]
colonized_prevdf_4weeks$ASV_ID <- rownames(colonized_prevdf_4weeks)
colonized_prevdf_4weeks <- colonized_prevdf_4weeks[,c(3,1,2)]
...
prevdf_5weeks <- makePrevDF(CHLD2_HMA_piglets_core_ps_5weeks,3)
prevdf_5weeks_table <- as.data.frame(prevdf_5weeks[1])
colonized_prevdf_5weeks <- prevdf_5weeks_table[prevdf_5weeks_table$Prevalence == 3,]
colonized_prevdf_5weeks$ASV_ID <- rownames(colonized_prevdf_5weeks)
colonized_prevdf_5weeks <- colonized_prevdf_5weeks[,c(3,1,2)]
...
prevdf_40days <- makePrevDF(CHLD2_HMA_piglets_core_ps_40days,3)
prevdf_40days_table <- as.data.frame(prevdf_40days[1])
colonized_prevdf_40days <- prevdf_40days_table[prevdf_40days_table$Prevalence == 3,]
colonized_prevdf_40days$ASV_ID <- rownames(colonized_prevdf_40days)
colonized_prevdf_40days <- colonized_prevdf_40days[,c(3,1,2)]
...
temporally_colonizing_core_taxa_CHLD2_piglets # 57 core ASVs 
...
actually_colonizing_core_taxa_CHLD2_piglets # 41 ASVs actually colonizing piglets

# persistent colonizers defined as taxa detected in all >= 4 time points
perst_colonizing_core_CHLD2_piglets_df <- subset(merged_prev_df_CHLD2_core_piglets, total >= '4') # select the ASVs which were detected in at least 4 time points
View(perst_colonizing_core_CHLD2_piglets_df)
perst_colonizing_core_CHLD2_piglets <- perst_colonizing_core_CHLD2_piglets_df[,1]
str(perst_colonizing_core_CHLD2_piglets) # 36 ASVs persistently colonizing piglets. These are the 'PERSISTENT COLONIZERS'
...
truly_colonizing_core_taxa_CHLD2_piglets # 30 ASVs 'truly' colonizing piglets
...
prevdf_48HRS <- makePrevDF(ADLT2_HMA_piglets_core_ps_48HRS,3) # there were only two fecal samples left for analysis for this time point since 'sample 105' was an outlier.
prevdf_48HRS_table <- as.data.frame(prevdf_48HRS[1])
colonized_prevdf_48HRS <- prevdf_48HRS_table[prevdf_48HRS_table$Prevalence >= 3,]
colonized_prevdf_48HRS$ASV_ID <- rownames(colonized_prevdf_48HRS)
colonized_prevdf_48HRS <- colonized_prevdf_48HRS[,c(3,1,2)]
...
prevdf_1week <- makePrevDF(ADLT2_HMA_piglets_core_ps_1week,3)
prevdf_1week_table <- as.data.frame(prevdf_1week[1])
colonized_prevdf_1week <- prevdf_1week_table[prevdf_1week_table$Prevalence >= 3,]
colonized_prevdf_1week$ASV_ID <- rownames(colonized_prevdf_1week)
colonized_prevdf_1week <- colonized_prevdf_1week[,c(3,1,2)]
...
prevdf_2weeks <- makePrevDF(ADLT2_HMA_piglets_core_ps_2weeks,3)
prevdf_2weeks_table <- as.data.frame(prevdf_2weeks[1])
colonized_prevdf_2weeks <- prevdf_2weeks_table[prevdf_2weeks_table$Prevalence >= 3,]
colonized_prevdf_2weeks$ASV_ID <- rownames(colonized_prevdf_2weeks)
colonized_prevdf_2weeks <- colonized_prevdf_2weeks[,c(3,1,2)]
...
prevdf_3weeks <- makePrevDF(ADLT2_HMA_piglets_core_ps_3weeks,3)
prevdf_3weeks_table <- as.data.frame(prevdf_3weeks[1])
colonized_prevdf_3weeks <- prevdf_3weeks_table[prevdf_3weeks_table$Prevalence >= 3,]
colonized_prevdf_3weeks$ASV_ID <- rownames(colonized_prevdf_3weeks)
colonized_prevdf_3weeks <- colonized_prevdf_3weeks[,c(3,1,2)]
...
prevdf_4weeks <- makePrevDF(ADLT2_HMA_piglets_core_ps_4weeks,3)
prevdf_4weeks_table <- as.data.frame(prevdf_4weeks[1])
colonized_prevdf_4weeks <- prevdf_4weeks_table[prevdf_4weeks_table$Prevalence >= 3,]
colonized_prevdf_4weeks$ASV_ID <- rownames(colonized_prevdf_4weeks)
colonized_prevdf_4weeks <- colonized_prevdf_4weeks[,c(3,1,2)]
...
prevdf_5weeks <- makePrevDF(ADLT2_HMA_piglets_core_ps_5weeks,3)
prevdf_5weeks_table <- as.data.frame(prevdf_5weeks[1])
colonized_prevdf_5weeks <- prevdf_5weeks_table[prevdf_5weeks_table$Prevalence >= 3,]
colonized_prevdf_5weeks$ASV_ID <- rownames(colonized_prevdf_5weeks)
colonized_prevdf_5weeks <- colonized_prevdf_5weeks[,c(3,1,2)]
...
prevdf_40days <- makePrevDF(ADLT2_HMA_piglets_core_ps_40days,3)
prevdf_40days_table <- as.data.frame(prevdf_40days[1])
colonized_prevdf_40days <- prevdf_40days_table[prevdf_40days_table$Prevalence >= 3,]
colonized_prevdf_40days$ASV_ID <- rownames(colonized_prevdf_40days)
colonized_prevdf_40days <- colonized_prevdf_40days[,c(3,1,2)]
...
temporally_colonizing_core_taxa_ADLT2_piglets # 73 core ASVs 
...
actually_colonizing_core_taxa_ADLT2_piglets # 52 ASVs persistently colonizing piglets

# persistent colonizers defined as taxa detected in >= 4 time points
perst_colonizing_core_ADLT2_piglets_df <- subset(merged_prev_df_ADLT2_core_piglets, total >= '4') # select the ASVs which were detected in at least 4 time points
View(perst_colonizing_core_ADLT2_piglets_df)
perst_colonizing_core_ADLT2_piglets <- perst_colonizing_core_ADLT2_piglets_df[,1]
perst_colonizing_core_ADLT2_piglets # 44 ASVs persistently colonizing piglets. These are the 'PERSISTENT COLONIZERS'.
...
truly_colonizing_core_taxa_ADLT2_piglets # 36 ASVs 'truly' colonizing piglets
...
# Let's see how many core ELD2 donor ASVs were found in at least one ELD2 HMA piglet fecal sample 
prevdf_ELD2_core_piglets <- makePrevDF(ELD2_HMA_piglets_core_ps, 1)
prevdf_ELD2_core_piglets_table <- as.data.frame(prevdf_ELD2_core_piglets[1])
colonized_prevdf_ELD2_core_piglets <- prevdf_ELD2_core_piglets_table[prevdf_ELD2_core_piglets_table$Prevalence >= 1,]
View(colonized_prevdf_ELD2_core_piglets) # 92 of the 134 core ELD2 donor ASVs found in at least one ELD2 mouse fecal sample. These are the 'COLONIZERS'.
core_ELD2_taxa_colonizing_piglets <-prevdf_ELD2_core_piglets[2]
core_ELD2_taxa_colonizing_piglets <- unlist(core_ELD2_taxa_colonizing_piglets) 
core_ELD2_taxa_colonizing_piglets
ELD2_HMA_piglets_core_ps_48HRS <- subset_samples(ELD2_HMA_piglets_core_ps, Description == '48hrs_post_inoc')
...
prevdf_48HRS <- makePrevDF(ELD2_HMA_piglets_core_ps_48HRS,3) 
prevdf_48HRS_table <- as.data.frame(prevdf_48HRS[1])
colonized_prevdf_48HRS <- prevdf_48HRS_table[prevdf_48HRS_table$Prevalence >= 3,]
colonized_prevdf_48HRS$ASV_ID <- rownames(colonized_prevdf_48HRS)
colonized_prevdf_48HRS <- colonized_prevdf_48HRS[,c(3,1,2)]
...
prevdf_1week <- makePrevDF(ELD2_HMA_piglets_core_ps_1week,3)
prevdf_1week_table <- as.data.frame(prevdf_1week[1])
colonized_prevdf_1week <- prevdf_1week_table[prevdf_1week_table$Prevalence >= 3,]
colonized_prevdf_1week$ASV_ID <- rownames(colonized_prevdf_1week)
colonized_prevdf_1week <- colonized_prevdf_1week[,c(3,1,2)]
...
prevdf_2weeks <- makePrevDF(ELD2_HMA_piglets_core_ps_2weeks,3)
prevdf_2weeks_table <- as.data.frame(prevdf_2weeks[1])
colonized_prevdf_2weeks <- prevdf_2weeks_table[prevdf_2weeks_table$Prevalence >= 3,]
colonized_prevdf_2weeks$ASV_ID <- rownames(colonized_prevdf_2weeks)
colonized_prevdf_2weeks <- colonized_prevdf_2weeks[,c(3,1,2)]
...
prevdf_3weeks <- makePrevDF(ELD2_HMA_piglets_core_ps_3weeks,3)
prevdf_3weeks_table <- as.data.frame(prevdf_3weeks[1])
colonized_prevdf_3weeks <- prevdf_3weeks_table[prevdf_3weeks_table$Prevalence >= 3,]
colonized_prevdf_3weeks$ASV_ID <- rownames(colonized_prevdf_3weeks)
colonized_prevdf_3weeks <- colonized_prevdf_3weeks[,c(3,1,2)]
...
prevdf_4weeks <- makePrevDF(ELD2_HMA_piglets_core_ps_4weeks,3)
prevdf_4weeks_table <- as.data.frame(prevdf_4weeks[1])
colonized_prevdf_4weeks <- prevdf_4weeks_table[prevdf_4weeks_table$Prevalence >= 3,]
colonized_prevdf_4weeks$ASV_ID <- rownames(colonized_prevdf_4weeks)
colonized_prevdf_4weeks <- colonized_prevdf_4weeks[,c(3,1,2)]
...
prevdf_5weeks <- makePrevDF(ELD2_HMA_piglets_core_ps_5weeks,3)
prevdf_5weeks_table <- as.data.frame(prevdf_5weeks[1])
colonized_prevdf_5weeks <- prevdf_5weeks_table[prevdf_5weeks_table$Prevalence >= 3,]
colonized_prevdf_5weeks$ASV_ID <- rownames(colonized_prevdf_5weeks)
colonized_prevdf_5weeks <- colonized_prevdf_5weeks[,c(3,1,2)]
...
prevdf_40days <- makePrevDF(ELD2_HMA_piglets_core_ps_40days,3)
prevdf_40days_table <- as.data.frame(prevdf_40days[1])
colonized_prevdf_40days <- prevdf_40days_table[prevdf_40days_table$Prevalence >= 3,]
colonized_prevdf_40days$ASV_ID <- rownames(colonized_prevdf_40days)
colonized_prevdf_40days <- colonized_prevdf_40days[,c(3,1,2)]
...
temporally_colonizing_core_taxa_ELD2_piglets # 69 core ASVs 
...
actually_colonizing_core_taxa_ELD2_piglets # 37 ASVs actually colonizing piglets

# persistent colonizers defined as taxa detected in >= 4 time points
perst_colonizing_core_ELD2_piglets_df <- subset(merged_prev_df_ELD2_core_piglets, total >= '4') # select the ASVs which were detected in at least 4 time points
View(perst_colonizing_core_ELD2_piglets_df)
perst_colonizing_core_ELD2_piglets <- perst_colonizing_core_ELD2_piglets_df[,1]
str(perst_colonizing_core_ELD2_piglets) # 19 ASVs persistently colonizing piglets. These are the 'PERSISTENT COLONIZERS'.
...
str(truly_colonizing_core_taxa_ELD2_piglets) # 12 ASVs 'truly' colonizing piglets
```
...
str(union_core_donor_ASVs) # 251 core ASVs represented across all dobnors ('union')
...
str(all_core_firmicute_ASVs_perst_colonizing_mice) # 24 of the core 'union' firmicute ASVs have persistently colonized the mice
...
str(all_core_firmicute_ASVs_perst_colonizing_piglets) # of the core 'union' firmicute ASVs have persistently colonized the mice
